...
#ifdef SOCIOCEZ_64_DDR_CH_SELECTION
	int vdi_set_ddr_channel_option(unsigned long core_idx, vpu_ddr_channel *ddr_channels);
	int vdi_get_ddr_channel_option(unsigned long core_idx, vpu_ddr_channel *ddr_channels);

	void vdi_set_ddr_channel_memory_info(unsigned long core_idx, vpu_ddr_channel ddr_ch, unsigned long addr, unsigned int size);	
#endif
