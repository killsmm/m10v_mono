...
/* Global Data															*/
/*----------------------------------------------------------------------*/
static T_DD_XDMASNAP_COMMON			gXDMASNAP_Common;

static VOID ct_xdmasnap_callback(VOID)
{
	Ddim_Print(("XDMASNAP Callback\n"));
}

static INT32 ct_xdmasnap_process(UCHAR num)
{
	INT32                 ret = 0;
	UINT32                wait_mode = 0;
	USHORT                status = 0;
	...
	switch (num) {
		// TEST ID 01-01-01
		case 1:
			memset((void*)D_XDMASNAP_AUTO_TEST_ADR_DST, 0x00, D_XDMASNAP_AUTO_TEST_SIZE);
			...
			dma_ctrl_trns.trns_size       = D_XDMASNAP_AUTO_TEST_SIZE;
			dma_ctrl_trns.src_addr        = D_XDMASNAP_AUTO_TEST_ADR_SRC;
			dma_ctrl_trns.dst_addr        = D_XDMASNAP_AUTO_TEST_ADR_DST;
			...
			Ddim_Print(("Result = %d\n", memcmp((void*)D_XDMASNAP_AUTO_TEST_ADR_SRC, (void*)D_XDMASNAP_AUTO_TEST_ADR_DST, D_XDMASNAP_AUTO_TEST_SIZE)));
		...
		case 3:
			...
			dma_ctrl_trns.src_addr        = D_XDMASNAP_AUTO_TEST_ADR_SRC;
			dma_ctrl_trns.dst_addr        = D_XDMASNAP_AUTO_TEST_ADR_DST;
			dma_ctrl_trns.int_handler     = ct_xdmasnap_callback;
		...
		case 11:
			...
			dma_trns_size.src_addr = D_XDMASNAP_AUTO_TEST_ADR_SRC;
			dma_trns_size.dst_addr = D_XDMASNAP_AUTO_TEST_ADR_DST;
		...
		// TEST ID 01-02-11
		case 15:
			Dd_XDMASNAP_Copy_SDRAM_Sync(0, D_XDMASNAP_AUTO_TEST_ADR_SRC, D_XDMASNAP_AUTO_TEST_ADR_DST, D_XDMASNAP_AUTO_TEST_SIZE, D_DD_XDMASNAP_WAITMODE_EVENT);
			break;
			
		case 16:
			Dd_XDMASNAP_Copy_SDRAM_Async(0, D_XDMASNAP_AUTO_TEST_ADR_SRC, D_XDMASNAP_AUTO_TEST_ADR_DST, D_XDMASNAP_AUTO_TEST_SIZE, NULL);
...
static void ct_xdmac_snap_result_out(INT32 expect, INT32 ret)
{
	printf("\tExpect = 0x%x\n", expect);
	printf("\tReturn = 0x%x\n", ret);
...
static VOID ct_xdma_snap_test( VOID )
{	
	INT32 ret;
	...
	ct_xdmac_snap_result_out(D_DD_XDMASNAP_INPUT_PARAM_ERR, ret);
	...
	ret = Dd_XDMASNAP_Open(0, D_DDIM_USER_SEM_WAIT_POL);
	ct_xdmac_snap_result_out(D_DD_XDMASNAP_SEM_TIMEOUT, ret);
	printf("\n");
	...
	ct_xdmac_snap_result_out(D_DD_XDMASNAP_SYSTEM_ERR, ret);
	printf("\n");
	...
	ct_xdmac_snap_result_out(0, ret);
	printf("\n");
 ...
 *	+-----------------+----------+------------+----------+------------+------------+----------+----+----+-----+-----+-----------------------------------+
 *	| "open"          | ch       | wait_time  |          |            |            |          |    |    |     |     | Dd_XDMASNAP_Open()               |
 *	+-----------------+----------+------------+----------+------------+------------+----------+----+----+-----+-----+-----------------------------------+
...
VOID Ct_Dd_XDMASNAP_Main(int argc, char** argv)
{
	...
	if (strcmp(argv[1], "open") == 0) {
		...
		ret = Dd_XDMASNAP_Open(ch, wait_time);
		if (ret == 0) {
			Ddim_Print(("XDMASNAP Open OK\n"));
		}
		else {
			Ddim_Print(("XDMASNAP Open ERR. ret=0x%x\n", ret));
		}
	}
		...
		if (ret == 0) {
			...
			Ddim_Print(("XDMASNAP Close ERR. ret=0x%x\n", ret));
		}
	}
	else if (strcmp(argv[1], "ctrl_cmm") == 0) {
		// Set XE
		if (strcmp(argv[2], "0") == 0) {
			gXDMASNAP_Common.common_config.bit.XE = D_DD_XDMASNAP_XDACS_XE_DISABLE;
		}
		else if (strcmp(argv[2], "1") == 0) {
			gXDMASNAP_Common.common_config.bit.XE = D_DD_XDMASNAP_XDACS_XE_ENABLE;
		}
		else {
		...
		if (strcmp(argv[3], "0") == 0) {
			gXDMASNAP_Common.common_config.bit.CP = D_DD_XDMASNAP_XDACS_CP_FIXED;
		}
		else if (strcmp(argv[3], "1") == 0) {
			gXDMASNAP_Common.common_config.bit.CP = D_DD_XDMASNAP_XDACS_CP_ROTATED;
		}
		else {
		...
		ret = Dd_XDMASNAP_Ctrl_Common(&gXDMASNAP_Common);
		if (ret == 0) {
			Ddim_Print(("XDMASNAP Ctrl_Common OK\n"));
		}
		else {
			Ddim_Print(("XDMASNAP Ctrl_Common ERR. ret=0x%x\n", ret));
		}
	}
		...
		Dd_XDMASNAP_Set_Int_Handler(ch, ct_xdmasnap_callback);
	}
	else if (strcmp(argv[1], "auto") == 0) {
		/* ch number */
		num = atoi(argv[2]);
		
		ret = ct_xdmasnap_process(num);
		if (ret == 0) {
			Ddim_Print(("ct_xdmasnap_process OK\n"));
		}
		else {
			Ddim_Print(("ct_xdmasnap_process ERR. ret=0x%x\n", ret));
		}
	}
		...
		if(strcmp(argv[2], "0") == 0){
			...
			ret = Dd_XDMASNAP_Ctrl_Trns(0, &dma_ctrl_trns);
			if(ret != D_DDIM_OK){
				Ddim_Print(("Dd_XDMASNAP_Ctrl_Trns ERR : ret=0x%x\n", ret));
			}
		}
		else if(strcmp(argv[2], "1") == 0){
			dma_trns_size.trns_size       = 0;
			...
			ret = Dd_XDMASNAP_Set_Trns_Size(0,&dma_trns_size);
			if(ret != D_DDIM_OK){
				Ddim_Print(("Dd_XDMASNAP_Set_Trns_Size ERR : ret=0x%x\n", ret));
			}
		}
		else{
			Ddim_Print(("Parameter ERR. P3 \n"));
