...
#define M43_CMD_INIT            0x01
#define M43_CMD_StartMF         0x03
#define M43_CMD_ContAPapxABS    0x02
#define M43_CMD_ContLDdstABS    0x04
#define M43_CMD_AskLDDataM      0x05
#define M43_CMD_AskLDinf		0x06
...
#define D_DAC_POS_MAX			(1185)			// 10bit resolution (temporary, need calibrarte)


//  int32_t g_dac_pulse_max ; 
//  int32_t g_dac_pulse_min ;
//  int32_t g_zoom1_pulse_max ; 
//  int32_t g_zoom1_pulse_min;
//  int32_t g_zoom2_pulse_max; 
//  int32_t g_zoom2_pulse_min;



//  double g_dac_pos_coarse_fw_setp;
//  double g_dac_pos_coarse_bw_setp;
//  double g_dac_pos_fine_fw_setp;
//  double g_dac_pos_fine_bw_setp;


int32_t g_dac_pulse_max = 522; 
int32_t g_dac_pulse_min = 0;
// int32_t g_zoom1_pulse_max = 1247; 
// int32_t g_zoom1_pulse_min = 0;
// int32_t g_zoom2_pulse_max = 1068; 
// int32_t g_zoom2_pulse_min = 0;
double g_dac_pos_coarse_fw_setp = 7.0f;
double g_dac_pos_coarse_bw_setp = 7.0f;
double g_dac_pos_fine_fw_setp = 4.0f;
double g_dac_pos_fine_bw_setp = 4.0f;
...
#define D_DAC_POS_BW_INIT			(1185)	// Start position for backward (temporary, need fine tuning)
...
#define D_DAC_POS_FINE_FW_SETP		(2)	// fine search forward moviing step (temporary, need fine tuning)
#define D_DAC_POS_FINE_BW_SETP		(2)	// fine search backward moviing step (temporary, need fine tuning)
...
const T_CAMERA_FOCUS_DOF M43_DOF[E_CAMERA_FOCUS_DOF_MAX] =		// dof table (temporary, need calibrarte)
...
T_CAMERA_FOCUS_BASIC	M43_BASIC =
{
...
E_CAMERA_ERR_CODE	Focus_VCM_M43_Init(E_CAMERA_ID focus_id, T_CAMERA_FOCUS_PARAM* prm)
{
    int ret = 0;
	UCHAR c1[] = {M43_CMD_INIT};
	...
	/* 读取设备状�?*/
	UCHAR c2[] = {M43_CMD_AskLDDataM};
	UCHAR res[6] = {0};
	ret = i2c_read(c2, 1, res, 6);
	printf("M43_CMD_AskLDDataM ret = %d\n", ret);
	for (size_t i = 0; i < 6; i++)
	{
		printf("%02x ", res[i]);	
	}
	printf("\n");
	int dac_pulse_maxmin = 0;
	if ( ret == 0 ) // 读取设备数据成功
	{
		uint16_t pulse_maxmin = (res[2]) | (res[3] << 8);
		if ( pulse_maxmin )
		{
			dac_pulse_maxmin = pulse_maxmin;
		}
		//printf("pulse_maxmin = %d\n", pulse_maxmin);
	} 
	
	c2[0] = M43_CMD_AskLDinf;
	ret = i2c_read(c2, 1, res, 2);
	printf("M43_CMD_AskLDinf ret = %d\n", ret);
	...
	if ( ret == 0 ) // 读取设备数据成功
	{
		uint16_t pulse_min = (res[0]) | (res[1] << 8);
		if ( pulse_min )
		{
			g_dac_pulse_min = pulse_min;
		}
		//printf("pulse_min = %d\n", pulse_min);
	}
	// 最近距�?
	//if (g_dac_pulse_min > 150)
	{
		g_dac_pulse_min = 150;
	}
	printf("pulse_min = %d\n", g_dac_pulse_min);
	...
	//最远距�?
	g_dac_pulse_max = dac_pulse_maxmin + g_dac_pulse_min;
	printf("pulse_max = %d\n", g_dac_pulse_max);
	/* 换算粗步�?*/
	 g_dac_pos_coarse_fw_setp =  g_dac_pos_coarse_fw_setp * 1.0 / D_DAC_POS_MAX * g_dac_pulse_max;
	//g_dac_pos_coarse_fw_setp =  g_dac_pos_coarse_fw_setp * 1.0 /  g_dac_pulse_max;
	//g_dac_pos_coarse_fw_setp = g_dac_pos_coarse_fw_setp * g_dac_pulse_max / D_DAC_POS_MAX;
	g_dac_pos_coarse_bw_setp = g_dac_pos_coarse_fw_setp;
	// 最近距�?
	M43_BASIC.max_value = g_dac_pulse_max;
	//最远距�?
	M43_BASIC.coarse_fw_step_num = (g_dac_pulse_max - D_DAC_POS_FW_INIT) / g_dac_pos_coarse_fw_setp;
	M43_BASIC.coarse_bw_step_num = (g_dac_pulse_max - D_DAC_POS_MIN) / g_dac_pos_coarse_bw_setp;
	M43_BASIC.fine_fw_step_num = 0;
	M43_BASIC.fine_bw_step_num = 0;
	init_T_CAMERA_FOCUS_PARAM( focus_id, prm );

	// for (size_t i = 0; i < 5; i++)
	// {
	// 	/* code */
	// 	Focus_VCM_M43_ContAPapxABS(0x1C2);
	// 	OS_User_Dly_Tsk(30);
	// }
	Focus_VCM_M43_ContAPapxABS(0);
	OS_User_Dly_Tsk(25);

    return E_CAMERA_ERR_OK;
}
E_CAMERA_ERR_CODE	Focus_VCM_M43_SetDAC(E_CAMERA_ID focus_id, USHORT dac)
{
	UCHAR	data[2];
	
	if(dac > g_dac_pulse_max) dac = g_dac_pulse_max; // 10bit resolution
	
	//uint32_t m43_pulse = 2500 - (dac  * 2.395);
	uint32_t m43_pulse = dac ;//* 1.0 / D_DAC_POS_MAX * ( g_dac_pulse_max - 50 );
	// if (m43_pulse < 50) 
	// 	m43_pulse = 50;
	
	Focus_VCM_M43_ContLDdstABS(m43_pulse);
	gDacValue_Curr[focus_id] = dac;

	return E_CAMERA_ERR_OK;
}

E_CAMERA_ERR_CODE	Focus_VCM_M43_Move(E_CAMERA_ID focus_id, E_CAMERA_FOCUS_MOVE_DIR dir, BOOL is_coarse, USHORT* value)
{
	...
	switch(dir)
	{
		...
		#if 1
			if(is_coarse){
				step_val = g_dac_pos_coarse_fw_setp;
			}else{
				step_val = D_DAC_POS_FINE_FW_SETP;
			}
			
			if((gDacValue_Curr[focus_id] + step_val) > g_dac_pulse_max){
		...
		#else
			if(is_coarse){
				step_val = g_dac_pos_coarse_bw_setp;
			}else{
		...
		case E_CAMERA_FOCUS_MOVE_DIR_FORWARD_INIT:
			...
			gDacValue_Curr[focus_id] = g_dac_pulse_min;
		break;
		case E_CAMERA_FOCUS_MOVE_DIR_BACKWARD_INIT:
			gDacValue_Curr[focus_id] = g_dac_pulse_max;
			//gDacValue_Curr[focus_id] = D_DAC_POS_FW_INIT;
		break;
		default:
	...
	ret = Focus_VCM_M43_SetDAC(focus_id, gDacValue_Curr[focus_id]);
//	OS_User_Slp_Tsk();
	
	//ret = Focus_VCM_M43_ContLDdstABS(gDacValue_Curr[focus_id]);
...
E_CAMERA_ERR_CODE	Focus_VCM_M43_CalcDAC(E_CAMERA_ID focus_id, T_CAMERA_FOCUS_PARAM* prm, UCHAR convert)
{
	FLOAT work;
...
E_CAMERA_ERR_CODE	Focus_VCM_M43_GetDAC(E_CAMERA_ID focus_id, USHORT* value)
{
	*value = gDacValue_Curr[focus_id];

	return E_CAMERA_ERR_OK;
}
E_CAMERA_ERR_CODE	Focus_VCM_M43_StartMF(bool mf_stat)
{

    int ret = 0;
	UCHAR c1[2] = {M43_CMD_StartMF};
...
E_CAMERA_ERR_CODE	Focus_VCM_M43_ContAPapxABS(uint16_t targetAperure)
{

    int ret = 0;
	UCHAR c1[3] = {M43_CMD_ContAPapxABS};
...
E_CAMERA_ERR_CODE	Focus_VCM_M43_ContLDdstABS(uint16_t abs_distance)
{

    int ret = 0;
	UCHAR c1[2] = {M43_CMD_ContLDdstABS};
...
static E_CAMERA_ERR_CODE	init_T_CAMERA_FOCUS_PARAM( E_CAMERA_ID focus_id, T_CAMERA_FOCUS_PARAM* prm )
{
	...
	prm->mac_dac = g_dac_pulse_max;	// Please set this parameter by using Calibration data.
