...
#define D_GYRO_CHIPID				( 0x12 ) // WHOAMI value for ICM20602 or derivative
...
/* Global Data															*/
/*----------------------------------------------------------------------*/
static const UCHAR	g_gyro_icm20602_accel_fs_sel = ACCEL_FS_2G;
static const UCHAR	g_gyro_icm20602_gyro_fs_sel = GYRO_FS_1000;
...
FJ_GYROIF_READ_CTRL gGyroCtrl_Data =
{
	...
	.data_ctrl.baudrate				= 18000000,		 						// 10MHz (ch2 is max 50MHz. when SPICLK=216MHz, ICM20602 : 10MHz)
...
VOID Gyro_SPI_Read( UCHAR addr, UCHAR* data )
{
	...
	ret = FJ_SPI_Stop( D_GYRO_SPI_CH );
	if( ret != FJ_ERR_OK ) {
		printf("FJ_SPI_Stop ERROR! : %d\n", __LINE__);
	}
...
// Read SPI data
VOID Gyro_SPI_Read_N( UCHAR addr, UCHAR* data, UCHAR read_bytes )
{
	FJ_ERR_CODE	ret;
	UCHAR		send_data[20] = {0};
...
E_CAMERA_ERR_CODE Gyro_Init( VOID )
{
	...
	// check device ID
	Gyro_SPI_Read( WHO_AM_I, &read_data );
	...
	// reset device
	Gyro_SPI_Write( PWR_MGMT_1, PWR_RESET );

	OS_User_Dly_Tsk( 100 );	// 100ms

	// clear sleep mode bit (6)
	Gyro_SPI_Write( PWR_MGMT_1, PWR_CLK_PLL1 );

	// set gyro 3DB BW = 3281 Hz, Noise BW = 3451.0 Hz, sampling rate = 8 kHz, Temp 3DB BW = 4000 Hz
	Gyro_SPI_Write( CONFIG, DLPF(7) | FSYNC_GYRO_X | FIFO_REPLACE );

	// set sample rate divider to 1
	Gyro_SPI_Write( SMPLRT_DIV, 0 );

	// set gyro full scale range to +1000 DPS
	Gyro_SPI_Write( GYRO_CONFIG, FCHOICE_B_00 | g_gyro_icm20602_gyro_fs_sel );

	// set accelerometer full scale range to +2g
	Gyro_SPI_Write( ACCEL_CONFIG, g_gyro_icm20602_accel_fs_sel );

	// set accel 3DB BW = 44.8 Hz, Noise BW = 61.5 Hz, sampling rate = 1 kHz
	Gyro_SPI_Write( ACCEL_CONFIG2, DLPF(3) | ACCEL_FCHOICE_B_0 );

	// trigger interupt when INT_RAW_RDY appears
	Gyro_SPI_Write( INT_ENABLE, INT_RAW_RDY );

	// INT: clear on any read and signal FSYNC by interrupt
	Gyro_SPI_Write( INT_PIN_CFG, INT_FSYNC_EN | INT_RD2CLEAR | INT_PULSE | INT_HIGH );

	// disable FIFO output
	Gyro_SPI_Write( FIFO_EN, 0 );

	// enable FIFO
	Gyro_SPI_Write( USER_CTRL, CTRL_FIFO_EN );
	...
	// clear sleep mode bit (6)
	Gyro_SPI_Read( PWR_MGMT_1, &read_data );
	printf("PWR_MGMT_1[0x%02X]\n", read_data);

	// set gyro 3DB BW = 3281 Hz, Noise BW = 3451.0 Hz, sampling rate = 8 kHz, Temp 3DB BW = 4000 Hz
	Gyro_SPI_Read( CONFIG, &read_data );
	printf("CONFIG[0x%02X]\n", read_data);

	// set sample rate divider to 1
	Gyro_SPI_Read( SMPLRT_DIV, &read_data );
	printf("SMPLRT_DIV[0x%02X]\n", read_data);

	// set gyro full scale range to +1000 DPS
	Gyro_SPI_Read( GYRO_CONFIG, &read_data );
	printf("GYRO_CONFIG[0x%02X]\n", read_data);

	// set accelerometer full scale range to +2g
	Gyro_SPI_Read( ACCEL_CONFIG, &read_data );
	printf("ACCEL_CONFIG[0x%02X]\n", read_data);

	// set accel 3DB BW = 44.8 Hz, Noise BW = 61.5 Hz, sampling rate = 1 kHz
	Gyro_SPI_Read( ACCEL_CONFIG2, &read_data );
	printf("ACCEL_CONFIG2[0x%02X]\n", read_data);

	// trigger interupt when FSYNC appears
	Gyro_SPI_Read( INT_ENABLE, &read_data );
	printf("INT_ENABLE[0x%02X]\n", read_data);

	// INT: clear on any read and signal FSYNC by interrupt
	Gyro_SPI_Read( INT_PIN_CFG, &read_data );
	printf("INT_PIN_CFG[0x%02X]\n", read_data);

	// disable FIFO output
	Gyro_SPI_Read( FIFO_EN, &read_data );
	printf("FIFO_EN[0x%02X]\n", read_data);

	// enable FIFO
	Gyro_SPI_Read( USER_CTRL, &read_data );
	printf("USER_CTRL[0x%02X]\n", read_data);
#endif

	// enable gyro and accelerometer to output to FIFO
	Gyro_SPI_Write( FIFO_EN, FIFO_ACCEL | FIFO_GYRO );
...
E_CAMERA_ERR_CODE Gyro_Calc_Bias( ULONG time_ms, T_CAMERA_GYRO_MPU_DATA* bias )
{
	...
	// Calc
	for( i = 0; i < loop; i ++ ){
		Gyro_SPI_Read_N( ACCEL_OUT, read_data, (sizeof(read_data) / sizeof(read_data[0])) );
		...
		sum_g[0] += Gyro_GyroData2Gravity( read_data[ 9], read_data[10] );
		sum_g[1] += Gyro_GyroData2Gravity( read_data[11], read_data[12] );
		sum_g[2] += Gyro_GyroData2Gravity( read_data[13], read_data[14] );
		OS_User_Dly_Tsk(1);
	}
	for( i = 0; i < 3; i ++ ){
		sum_g[i] /= loop;
...
E_CAMERA_ERR_CODE Gyro_Sleep( VOID )
{
	...
	// disable interupt
	Gyro_SPI_Write( INT_ENABLE, 0 );

	// sleep device
	Gyro_SPI_Write( PWR_MGMT_1, PWR_SLEEP );
...
E_CAMERA_ERR_CODE Gyro_Get_Data(INT32 read_cnt, T_CAMERA_GYRO_MPU_DATA* data )
{
	...
	mpu_data.gyro[x-1] = sx * Gyro_GyroData2Gravity( gGyroCtrl_Recv_Data[ read_cnt ][ 9], gGyroCtrl_Recv_Data[ read_cnt ][10] ) * ( M_PI / 180 );
	mpu_data.gyro[y-1] = sy * Gyro_GyroData2Gravity( gGyroCtrl_Recv_Data[ read_cnt ][11], gGyroCtrl_Recv_Data[ read_cnt ][12] ) * ( M_PI / 180 );
	mpu_data.gyro[z-1] = sz * Gyro_GyroData2Gravity( gGyroCtrl_Recv_Data[ read_cnt ][13], gGyroCtrl_Recv_Data[ read_cnt ][14] ) * ( M_PI / 180 );
...
FLOAT Gyro_AccelData2Gravity( const UCHAR data_h, const UCHAR data_l )
{
	...
	switch( g_gyro_icm20602_accel_fs_sel ) {
		...
		case ACCEL_FS_16G:	// +-16g Full scale.
			...
			break;
		default:
			printf( "%s(): unknown ACCEL_FS_SEL. 0x%x\n", __func__, g_gyro_icm20602_accel_fs_sel );	/* pgr0822 */
...
FLOAT Gyro_GyroData2Gravity( const UCHAR data_h, const UCHAR data_l )
{
	...
	switch( g_gyro_icm20602_gyro_fs_sel ) {
		...
		case GYRO_FS_2000:	// +-2000 degree/sec Full scale.
			...
			break;
		default:
			printf( "%s(): unknown GYRO_FS_SEL. 0x%x\n", __func__, g_gyro_icm20602_gyro_fs_sel );	/* pgr0822 */
...
VOID Gyro_Dump( VOID )
{
	...
	data_f = Gyro_GyroData2Gravity( read_data[13], read_data[14] );
	printf("GYRO_ZOUT [0x%02X%02X=%9.4fdeg/sec]\n", read_data[13], read_data[14], data_f );
}
