...
/* Global Data															*/
/*----------------------------------------------------------------------*/
static const UCHAR	g_gyro_mpu9250_accel_fs_sel = ACCEL_FS_2G;
static const UCHAR	g_gyro_mpu9250_gyro_fs_sel = GYRO_FS_1000;
...
FJ_GYROIF_READ_CTRL gGyroCtrl_Data =
{
	...
	.data_ctrl.baudrate				= 18000000,		 						// 18MHz (ch2 is max 50MHz. when SPICLK=216MHz, div=5, MPU9250 read is max 20MHz)
...
E_CAMERA_ERR_CODE Gyro_Init( VOID )
{
	...
	// set gyro sampling rate to 8 kHz, bandwidth to 3.6kHz, and delay to 0.17 ms
	...
	Gyro_SPI_Write( GYRO_CONFIG, FCHOICE_11 | g_gyro_mpu9250_gyro_fs_sel );

	// set accelerometer full scale range to +2g
	Gyro_SPI_Write( ACCEL_CONFIG, g_gyro_mpu9250_accel_fs_sel );

	// set accel sampling rate to 1 kHz, bandwidth to 41Hz, and delay to 11.8 ms
	Gyro_SPI_Write( ACCEL_CONFIG2, DLPF(3) | AFCHOICE_1 );
...
FLOAT Gyro_AccelData2Gravity( const UCHAR data_h, const UCHAR data_l )
{
	...
	switch( g_gyro_mpu9250_accel_fs_sel ) {
		...
		case ACCEL_FS_16G:	// +-16g Full scale.
			...
			break;
		default:
			printf( "%s(): unknown ACCEL_FS_SEL. 0x%x\n", __func__, g_gyro_mpu9250_accel_fs_sel );	/* pgr0822 */
...
FLOAT Gyro_GyroData2Gravity( const UCHAR data_h, const UCHAR data_l )
{
	...
	switch( g_gyro_mpu9250_gyro_fs_sel ) {
		...
		case GYRO_FS_2000:	// +-2000 degree/sec Full scale.
			...
			break;
		default:
			printf( "%s(): unknown GYRO_FS_SEL. 0x%x\n", __func__, g_gyro_mpu9250_gyro_fs_sel );	/* pgr0822 */
