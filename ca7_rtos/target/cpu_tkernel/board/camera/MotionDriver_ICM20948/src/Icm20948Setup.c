 ...
 *
 * This software, related documentation and any modifications thereto (collectively �oftware��) is subject
			...
			while(total_sample_cnt--) {
				...
				if (header & GYRO_SET) {
					...
					if(inv_icm20948_ctrl_androidSensor_enabled(s, ANDROID_SENSOR_GYROSCOPE) && !skip_sensor(s, ANDROID_SENSOR_GYROSCOPE)) {
						...
						/* send raw float gyro */
//						inv_icm20948_get_sen_info(gyro_float, INV_ICM20948_SENSOR_GYROSCOPE);
					}
				...
				if (header & ACCEL_SET) {
					float scale;
					/* Read calibrated accel out of DMP FIFO and convert it from Q25 raw data format to m/s2 in Android format */
					...
					if((inv_icm20948_ctrl_androidSensor_enabled(s, ANDROID_SENSOR_ACCELEROMETER) && !skip_sensor(s, ANDROID_SENSOR_ACCELEROMETER)) ||
					   (inv_icm20948_ctrl_androidSensor_enabled(s, ANDROID_SENSOR_LINEAR_ACCELERATION))) {
						...
						if(inv_icm20948_ctrl_androidSensor_enabled(s, ANDROID_SENSOR_ACCELEROMETER)) {
							...
							/* send raw float accel */
//							inv_icm20948_get_sen_info(accel_float, INV_ICM20948_SENSOR_ACCELEROMETER);
						}
					}
				}
				...
				if (header & CPASS_CALIBR_SET) {
					...
					/* Read calibrated compass out of DMP FIFO and convert it from Q16 raw data format to 弮T in Android format */
					...
					if(inv_icm20948_ctrl_androidSensor_enabled(s, ANDROID_SENSOR_GEOMAGNETIC_FIELD) && !skip_sensor(s, ANDROID_SENSOR_GEOMAGNETIC_FIELD)) {
						...
						/* send raw float compass */
//						inv_icm20948_get_sen_info(compass_float, INV_ICM20948_SENSOR_GEOMAGNETIC_FIELD);
					}
				}
				...
				if (header & QUAT9_SET) {
					...
					if(inv_icm20948_ctrl_androidSensor_enabled(s, ANDROID_SENSOR_ORIENTATION) && !skip_sensor(s, ANDROID_SENSOR_ORIENTATION)) {
						...
						/* send raw float orientation */
//						inv_icm20948_get_sen_info(orientation_float, INV_ICM20948_SENSOR_ORIENTATION);
