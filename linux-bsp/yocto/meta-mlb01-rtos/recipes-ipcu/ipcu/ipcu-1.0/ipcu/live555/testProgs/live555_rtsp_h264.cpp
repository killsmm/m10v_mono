...
{
	printf("Usage:\n");
	printf("live555_rtsp_h264 <Param1>\n");
	printf("  Param1: pcm or aac or none\n");

	printf("Example:\n");
	printf("  live555_rtsp_h264 pcm\n");
}
...
int main(int argc, char** argv) {
  ...
  portNumBits rtspServerPortNum = 8557; // rtsp port
  ...
  sms->addSubsession(LadiH264SubSession::createNew(*env, VIDEO_H264));
  if (audio_fmt != D_AUDIO_NONE) {
	  sms->addSubsession(LadiAudioSubSession::createNew(*env, VIDEO_H264, audio_fmt));
  }
