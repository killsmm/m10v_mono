...
static const struct uvc_frame_info uvc_frames_mjpeg[] = {
	{ "UHD",   3840, 2160, { 10000000/ 4, 0 } },
	{ "FHD",   1920, 1080, { 10000000/15, 0 } },
	{ "HD",    1280,  720, { 10000000/30, 0 } },
	{ "QUXGA", 3200, 2400, { 10000000/ 4, 0 } },
	{ "UXGA",  1600, 1200, { 10000000/15, 0 } },
	{ "XGA",   1024,  768, { 10000000/30, 0 } },
	{ "VGA",    640,  480, { 10000000/30, 0 } },
	{  0,         0,    0, { 0 } },
};

static const struct uvc_frame_info uvc_frames_yuyv[] = {
	{ "UHD",   3840, 2160, { 10000000/ 2, 0 } },
	{ "FHD",   1920, 1080, { 10000000/ 5, 0 } },
	{ "HD",    1280,  720, { 10000000/15, 0 } },
	{ "QUXGA", 3200, 2400, { 10000000/ 2, 0 } },
	{ "UXGA",  1600, 1200, { 10000000/ 5, 0 } },
	{ "XGA",   1024,  768, { 10000000/15, 0 } },
	{ "VGA",    640,  480, { 10000000/30, 0 } },
	{  0,         0,    0, { 0 } },
};
...
static const struct uvc_frame_info ss_uvc_frames_yuyv[] = {
	{ "UHD",   3840, 2160, { 10000000/15, 0 } },
	{ "FHD",   1920, 1080, { 10000000/30, 0 } },
	{ "HD",    1280,  720, { 10000000/30, 0 } },
	{ "QUXGA", 3200, 2400, { 10000000/15, 0 } },
	{ "UXGA",  1600, 1200, { 10000000/60, 0 } },
	{ "XGA",   1024,  768, { 10000000/60, 0 } },
...
static const struct uvc_frame_info ss_uvc_frames_mjpeg[] = {
	...
	{ "FHD",   1920, 1080, { 10000000/60, 0 } },
	{ "HD",    1280,  720, { 10000000/60, 0 } },
	{ "QUXGA", 3200, 2400, { 10000000/30, 0 } },
	{ "UXGA",  1600, 1200, { 10000000/60, 0 } },
	{ "XGA",   1024,  768, { 10000000/60, 0 } },
	{ "VGA",    640,  480, { 10000000/60, 0 } },
	{  0,         0,    0, { 0 } },
...
{
	unsigned int codec_type, capture_size, usecase, framerate;
	struct camera_if_cmd cmd;

	if (dev->udev->fcc == V4L2_PIX_FMT_YUYV)
		codec_type = 3;
	else if (dev->udev->fcc == V4L2_PIX_FMT_MJPEG)
		codec_type = 2;
	else if (dev->udev->fcc == V4L2_PIX_FMT_H264)
		codec_type = 1;
	else
		return -1;
	...
	if ((dev->udev->width == 3840) && (dev->udev->height == 2160)) {
		capture_size = 0;
		usecase = 0;
	} else if ((dev->udev->width == 3200) && (dev->udev->height == 2400)) {
		capture_size = 1;
		usecase = 6;
	} else if ((dev->udev->width == 1920) && (dev->udev->height == 1080)) {
		capture_size = 2;
		usecase = 1;
	} else if ((dev->udev->width == 1600) && (dev->udev->height == 1200)) {
		capture_size = 3;
		usecase = 7;
	} else if ((dev->udev->width == 1280) && (dev->udev->height == 720)) {
		capture_size = 4;
		usecase = 2;
	} else if ((dev->udev->width == 1024) && (dev->udev->height == 768)) {
		capture_size = 5;
		usecase = 8;
	...
	cmd.cmd_set = E_UVC_IFT_COM_SET_MODE_CHANGE;
	cmd.cmd = E_UVC_IFT_SUB_COM_CHANGE_MODE;
	cmd.send_param[0] = 1;
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("a_camera IF error.\n");
		goto err;
	}

	// usecase select
	memset(&cmd, 0, sizeof(cmd));
	cmd.cmd_set = E_UVC_IFT_COM_SET_PRE_CFG;
	cmd.cmd = 0x00000300;
	cmd.send_param[0] = usecase;
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("b_camera IF error.\n");
		goto err;
	}
	dev->usecase = usecase;

	if (codec_type != dev->codec_type) {
		memset(&cmd, 0, sizeof(cmd));
		cmd.cmd_set = E_UVC_IFT_COM_SET_VIDEO_CAP_CFG;
		cmd.cmd = E_UVC_IFT_SUB_COM_V_CAP_ENABLE;
		cmd.send_param[0] = dev->codec_type;
		cmd.send_param[1] = 0;
		cmd.to_sec = 3;
		if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
			printf("c_camera IF error.\n");
			goto err;
		}
	}
	...
	cmd.cmd_set = E_UVC_IFT_COM_SET_VIDEO_CAP_CFG;
	cmd.cmd = E_UVC_IFT_SUB_COM_V_CAP_ENABLE;
	cmd.send_param[0] = codec_type;
	cmd.send_param[1] = 1;
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("d_camera IF error.\n");
		goto err;
	}
	dev->codec_type = codec_type;

	// Capture size
	memset(&cmd, 0, sizeof(cmd));
	cmd.cmd_set = E_UVC_IFT_COM_SET_VIDEO_CAP_CFG;
	cmd.cmd = E_UVC_IFT_SUB_COM_V_CAP_SIZE;
	cmd.send_param[0] = codec_type;
	cmd.send_param[1] = capture_size;
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("e_camera IF error.\n");
		goto err;
	}
	dev->capture_size = capture_size;

	// Frame rate
	memset(&cmd, 0, sizeof(cmd));
	cmd.cmd_set = E_UVC_IFT_COM_SET_VIDEO_CAP_CFG;
	cmd.cmd = E_UVC_IFT_SUB_COM_V_CAP_FRAMERATE;
	cmd.send_param[0] = codec_type;
	cmd.send_param[1] = framerate;
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("f_camera IF error.\n");
		goto err;
	}
	dev->framerate = framerate;
	...
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("g_camera IF error.\n");
		goto err;
	}

	// Change VideoCapture Mode
	memset(&cmd, 0, sizeof(cmd));
	cmd.cmd_set = E_UVC_IFT_COM_SET_MODE_CHANGE;
	cmd.cmd = E_UVC_IFT_SUB_COM_CHANGE_MODE;
	cmd.send_param[0] = 8;
...
ipcu_open(struct ipcu_device **idev)
{
...
#if 1
	memset(&cmd, 0, sizeof(cmd));
	cmd.cmd_set = E_UVC_IFT_COM_INIT_SETTING;
	cmd.cmd = E_UVC_IFT_SUB_COM_FINISH_BOOT;
	...
	cmd.cmd = E_UVC_IFT_SUB_COM_IMG_ZOOM;
	cmd.send_param[0] = 1;
	cmd.to_sec = 3;
	if (camera_if_command(&cmd) != 0 || cmd.recv_param[0] != 0) {
		printf("camera IF error.\n");
			   ...
			   struct uvc_request_data *resp)
{
	...
	case 2:
		...
		case UVC_PU_GAMMA_CONTROL:
			...
			uvc_control(req, dev, resp, &gamma);
			break;
				...
				struct uvc_request_data *data)
{
	...
	case 2:
		...
		case UVC_PU_GAMMA_CONTROL:
			debug_p(">>> UVC_PU_GAMMA_CONTROL\n");
			memcpy(&gamma.val, data->data, data->length);
...
main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "b:p:u:h")) != -1) {
		switch (opt) {
		case 'b':
