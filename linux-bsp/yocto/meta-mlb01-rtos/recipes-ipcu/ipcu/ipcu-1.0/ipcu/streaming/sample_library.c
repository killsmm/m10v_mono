...
static sem_t cb_sem;
static sem_t cmd_if_sem;
static volatile int g_cmd_if_sem_init = 0;
		    ...
		    UINT32 total_length)
{
	...
	if (p->MagicCode == 0x9999EEEE) {
		...
		if (ret > 0) {
			printf("callback_camif ret:%d failed\n", ret);
			//return;
		}
		/* response process */
		release_stream(p);
				       ...
				       UINT32 total_length)
{
	...
	if (camera_if_cmd_rcv_id != id ||
		...
		if (sem_post(&cb_sem) == -1) {
			handle_error("sem_post(&cb_sem)");
		}
...
static int camera_if_send_cmd(struct camera_if_cmd* cam_if_cmd)
{
	...
	timeout.tv_nsec = 0;
	if (sem_timedwait(&cb_sem, &timeout) == -1) {
		handle_error("sem_timedwait(&cb_sem, &timeout)");
		ret = -1;
		goto end;
	}
	...
	ret = FJ_IPCU_Send(camera_if_cmd_send_id,
			   ...
			   IPCU_SEND_SYNC);
	if (ret != FJ_ERR_OK) {
		printf("ipcu send failed , ret:%08x\n", ret);
		goto end;
	}
...
int camera_if_command(struct camera_if_cmd* cam_if_cmd)
{
	...
	if (g_cmd_if_sem_init == 0) {
		if (sem_init(&cmd_if_sem, 1, 1) == -1) {
			handle_error("sem_init(&cmd_if_sem)");
		}
		g_cmd_if_sem_init = 1;
	}
	...
	timeout.tv_nsec = 0;
	if (sem_timedwait(&cmd_if_sem, &timeout) == -1) {
		handle_error("sem_timedwait(&cmd_if_sem, &timeout)");
		ret = -1;
		goto end;
	}
	...
	if (sem_init(&cb_sem, 0, 1) == -1) {
		handle_error("sem_init(&cb_sem, 0, 1)");
		goto end;
	}
	...
	if (sem_destroy(&cb_sem) == -1) {
		handle_error("sem_destroy(&cb_sem)");
	}

	camera_if_ipcu_ch_close();

	if (sem_post(&cmd_if_sem) == -1) {
		handle_error("sem_post(&cmd_if_sem)");
	}
