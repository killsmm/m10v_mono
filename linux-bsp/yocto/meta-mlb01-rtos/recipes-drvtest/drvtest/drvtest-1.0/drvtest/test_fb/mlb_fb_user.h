#ifndef MIL_FB_USER_H
#define MIL_FB_USER_H
#define FBIOSWICH_DROW_BUFFER (0x5000) /* Switch drawing buffer by ioctl */
#define FBIOENABLE_DROW_AREA	(0x5001)	/* Enable/Disable drawing area  */
#define FBIOFULL_SETTING	(0x5002)	/* Set full GR  */
#define FBIOSET_TRIGER	(0x5003)	/* Set GR's triger ON/OFF */
#define FBIOSWITCH_GR_SETTING	(0x5004)	/* Switch GR's setting */
#define FBIOSET_DRAWAREA	(0x5005)	/* Set every drawing area's position and size */
#define FBIOGET_DRAWAREA	(0x5006)	/* Get every drawing area's position and size */
#define FBIOFULL_GETTING	(0x5007)	/* Get full GR  */
#define FBIOPUT_TEST_VSCREENINFO	(0x5500)	/* Test FBIOPUT_VSCREENINFO */
#define FBIOPUT_TEST_IPO	(0x5501)	/* Test IPO */
...
union gridt{
    unsigned long       word;
    struct {
        unsigned long   ifmt    :3;/* Image data format. 0: RGBA8888, 2: RGBA4444 */
        unsigned long           :1;
...
typedef struct {
	union grisize	input_image_size[MAX_DRAW_AREA_NUM];	/* Every input image area's size */ 
	union grdsta	input_image_position[MAX_DRAW_AREA_NUM];	/* Every input image area's position */
	int	enable_show_area[MAX_DRAW_AREA_NUM];
	unsigned long	input_image_horizonal_bytes[MAX_DRAW_AREA_NUM];	/* Every input image area's horizonal width */
...
struct fb_disp {
	union gridt		iput_trans_setting;	/* Image data format. 0: RGBA8888, 2: RGBA4444, 5: Ycc+A4 6: Ycc+A8 */
	...
	u32 pan_step[2];/* [0]: Pan step of X. [1]:Pan step of Y. They are as same as xpanstep/ypanstep*/	
	char setting_name[10];

	unsigned long	input_image_horizonal_bytes[MAX_DRAW_AREA_NUM];	/* Every input image area's horizonal width */
#ifdef SUPORT_YUV_FORMAT
	unsigned long	A_data_horizonal_bytes[MAX_DRAW_AREA_NUM];	/* A data's  horizonal width. It is valid at YCC+A4/A8 only */
	struct grimage_address	input_A_data_ph_address[MAX_DRAW_AREA_NUM];	/* Every input A data's physical address  */
	struct grimage_address	input_A_data_address[MAX_DRAW_AREA_NUM];	/* Every input A data's address  */
#endif
	struct grimage_address	input_image_ph_address[MAX_DRAW_AREA_NUM];	/* Every input image's physical address  */
};
