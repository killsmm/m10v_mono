...
static const struct uvc_format_info uvc_formats[] = {
	{ V4L2_PIX_FMT_H264, uvc_frames_h264 }, /* keep at top because UVC exposes only H.264 */
 ...
 * ftrace staff
 */
static const char ftrace_marker[] = "/sys/kernel/debug/tracing/trace_marker";
static FILE *ftracefp = NULL;

static void init_ftrace(void)
{
	if (ftracefp)
		fclose(ftracefp);
	ftracefp = fopen(ftrace_marker, "w");
	if (!ftracefp)
		fprintf(stderr, "Error: failed to open ftrace_marker\n");
	else
		fprintf(stderr, "OK, now ftrace_marker is opened\n");
}

static int vftprintf(const char *fmt, va_list args)
{
	int ret;

	ret = vfprintf(stderr, fmt, args);
	if (ftracefp)
		ret = vfprintf(ftracefp, fmt, args);
	return ret;
}

/* Non flush mode */
static int __ftprintf(const char *fmt, ...)
{
	va_list args;
	int ret;

	va_start(args, fmt);
	ret = vftprintf(fmt, args);
	va_end(args);
	return ret;
}

static int ftprintf(const char *fmt, ...)
{
	va_list args;
	int ret;
...
v4l2_reqbufs_mmap(struct v4l2_device *dev, int nbufs)
{
	...
	if (ret < 0) {
		if (ret == -EINVAL)
			ftprintf("V4L2: does not support memory mapping\n");
		else
			ftprintf("V4L2: VIDIOC_REQBUFS error %s (%d).\n",
	...
	if (req.count < 2) {
		ftprintf("V4L2: Insufficient buffer memory.\n");
		ret = -EINVAL;
		goto err;
	}
	...
	for (i = 0; i < req.count; ++i) {
		...
		if (ret < 0) {
			ftprintf("V4L2: VIDIOC_QUERYBUF failed for buf %d: "
		...
		if (MAP_FAILED == dev->mem[i].start) {
			ftprintf("V4L2: Unable to map buffer %u: %s (%d).\n", i,
				strerror(errno), errno);
			dev->mem[i].length = 0;
			ret = -EINVAL;
			goto err_free;
		...
		ftprintf("V4L2: Buffer %u mapped at address %p.\n", i,
				dev->mem[i].start);
	}

	dev->nbufs = req.count;
	ftprintf("V4L2: %u buffers allocated.\n", req.count);
...
v4l2_reqbufs_userptr(struct v4l2_device *dev, int nbufs)
{
	...
	if (ret < 0) {
		if (ret == -EINVAL)
			ftprintf("V4L2: does not support user pointer i/o\n");
		else
			ftprintf("V4L2: VIDIOC_REQBUFS error %s (%d).\n",
					strerror(errno), errno);
		return ret;
...
v4l2_qbuf_mmap(struct v4l2_device *dev)
{
	...
	for (i = 0; i < dev->nbufs; ++i) {
		...
		if (ret < 0) {
			ftprintf("V4L2: VIDIOC_QBUF failed : %s (%d).\n",
					strerror(errno), errno);
			return ret;
		}
...
v4l2_process_data(struct v4l2_device *dev)
{
	...
	if (dev->udev->first_buffer_queued)
		if (dev->dqbuf_count >= dev->qbuf_count) {
			ftprintf("Skip: buffer already queued.\n");
			return 0;
		}
	...
	ftprintf("Dequeueing buffer at V4L2 side = %d\n", vbuf.index);
#endif
...
#ifdef ENABLE_BUFFER_DEBUG
	ftprintf("Queueing buffer at UVC side = %d\n", ubuf.index);
#endif
...
v4l2_get_format(struct v4l2_device *dev)
{
	...
	if (ret < 0) {
		ftprintf("V4L2: Unable to get format: %s (%d).\n",
			strerror(errno), errno);
		return ret;
	}

	ftprintf("V4L2: Getting current format: %c%c%c%c %ux%u\n",
...
v4l2_set_format(struct v4l2_device *dev, struct v4l2_format *fmt)
{
	...
	if (ret < 0) {
		ftprintf("V4L2: Unable to set format %s (%d).\n",
			strerror(errno), errno);
		return ret;
	}

	ftprintf("V4L2: Setting format to: %c%c%c%c %ux%u\n",
		pixfmtstr(fmt->fmt.pix.pixelformat),
		fmt->fmt.pix.width, fmt->fmt.pix.height);
...
v4l2_set_ctrl(struct v4l2_device *dev, int new_val, int ctrl)
{
	...
	case V4L2_CID_BRIGHTNESS:
		...
		if (-1 == ret) {
			if (errno != EINVAL)
				ftprintf("V4L2: VIDIOC_QUERYCTRL"
					" failed: %s (%d).\n",
					strerror(errno), errno);
			else
				ftprintf ("V4L2_CID_BRIGHTNESS is not"
					" supported: %s (%d).\n",
					strerror(errno), errno);

			return ret;
		} else if (queryctrl.flags & V4L2_CTRL_FLAG_DISABLED) {
			ftprintf ("V4L2_CID_BRIGHTNESS is not supported.\n");
			ret = -EINVAL;
			return ret;
		} else {
			CLEAR(control);
			...
			if (-1 == ret) {
				ftprintf("V4L2: VIDIOC_S_CTRL failed: %s (%d).\n",
					strerror(errno), errno);
				return ret;
			}
		}
		ftprintf ("V4L2: Brightness control changed to value = 0x%x\n",
				new_val);
		break;
...
v4l2_start_capturing(struct v4l2_device *dev)
{
	...
	if (ret < 0) {
		ftprintf("V4L2: Unable to start streaming: %s (%d).\n",
			strerror(errno), errno);
		return ret;
	}

	ftprintf("V4L2: Starting video stream.\n");
...
v4l2_stop_capturing(struct v4l2_device *dev)
{
	...
	case IO_METHOD_MMAP:
		...
		if (ret < 0) {
			ftprintf("V4L2: VIDIOC_STREAMOFF failed: %s (%d).\n",
					strerror(errno), errno);
			return ret;
		}
...
v4l2_open(struct v4l2_device **v4l2, char *devname, struct v4l2_format *s_fmt)
{
	...
	if (fd == -1) {
		ftprintf("V4L2: device open failed: %s (%d).\n",
				strerror(errno), errno);
		return ret;
	}
	...
	if (ret < 0) {
		ftprintf ("V4L2: VIDIOC_QUERYCAP failed: %s (%d).\n",
				strerror(errno), errno);
		goto err;
	}

	if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
		ftprintf("V4L2: %s is no video capture device\n", devname);
		goto err;
	}

	if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
		ftprintf("V4L2: %s does not support streaming i/o\n",
					devname);
		goto err;
	}
	...
	ftprintf("V4L2 device is %s on bus %s\n", cap.card, cap.bus_info);
	...
	ftprintf("v4l2 open succeeded, file descriptor = %d\n", fd);
...
uvc_video_stream(struct uvc_device *dev, int enable)
{
	...
	if (!enable) {
		...
		if (ret < 0) {
			ftprintf("UVC: VIDIOC_STREAMOFF failed: %s (%d).\n",
					strerror(errno), errno);
			return ret;
		}

		ftprintf("UVC: Stopping video stream.\n");
	...
	if (ret < 0) {
		ftprintf("UVC: Unable to start streaming %s (%d).\n",
			strerror(errno), errno);
		return ret;
	}

	ftprintf("UVC: Starting video stream.\n");
...
uvc_open(struct uvc_device **uvc, char *devname)
{
	...
	if (fd == -1) {
		ftprintf("UVC: device open failed: %s (%d).\n",
				strerror(errno), errno);
		return ret;
	}
	...
	ret = ioctl(fd, VIDIOC_QUERYCAP, &cap);
	if (ret < 0) {
		ftprintf("UVC: unable to query uvc device: %s (%d)\n",
				strerror(errno), errno);
		goto err;
	}

	if (!(cap.capabilities & V4L2_CAP_VIDEO_OUTPUT)) {
		ftprintf("UVC: %s is no video output device\n", devname);
		goto err;
	}
	...
	ftprintf("uvc device is %s on bus %s\n", cap.card, cap.bus_info);
	ftprintf("uvc open succeeded, file descriptor = %d\n", fd);
...
uvc_video_process(struct uvc_device *dev)
{
	...
	if (dev->run_standalone) {
...
#ifdef ENABLE_BUFFER_DEBUG
		ftprintf("DeQueued buffer at UVC side = %d\n", ubuf.index);
#endif
		uvc_video_fill_buffer(dev, &ubuf);

		ret = ioctl(dev->uvc_fd, VIDIOC_QBUF, &ubuf);
		if (ret < 0) {
			ftprintf("UVC: Unable to queue buffer: %s (%d).\n",
					strerror(errno), errno);
			return ret;
		}
...
#ifdef ENABLE_BUFFER_DEBUG
		ftprintf("ReQueueing buffer at UVC side = %d\n", ubuf.index);
#endif
	} else {
		...
		if (!dev->vdev->is_streaming || !dev->first_buffer_queued) {
			ftprintf("Error: V4L2 streaming is not started or QBUF was not called.\n");
			return 0;
		}
		...
		if (!dev->uvc_shutdown_requested)
			if ((dev->dqbuf_count + 1) >= dev->qbuf_count) {
				ftprintf("Skip: at least 2 buffers needed\n");
				return 0;
			}
		...
		ret = ioctl(dev->uvc_fd, VIDIOC_DQBUF, &ubuf);
		if (ret < 0) {
			ftprintf("Error: failed to dequeue buffer:%d\n", errno);
			return ret;
		}
		...
		if (ret < 0) {
			ftprintf("V4L2: Unable to queue buffer: %s (%d).\n",
					strerror(errno), errno);
			return ret;
		}
...
#ifdef ENABLE_BUFFER_DEBUG
		ftprintf("ReQueueing buffer at V4L2 side = %d\n", vbuf.index);
#endif
	}
...
uvc_video_qbuf_mmap(struct uvc_device *dev)
{
	...
	ftprintf("UVC: VIDIOC_QBUF mmap: %d\n", dev->nbufs);
	for (i = 0; i < dev->nbufs; ++i) {
		memset(&dev->mem[i].buf, 0, sizeof(dev->mem[i].buf));
		...
		if (ret < 0) {
			ftprintf("UVC: VIDIOC_QBUF failed : %s (%d).\n",
					strerror(errno), errno);
			return ret;
		}
...
uvc_video_qbuf_userptr(struct uvc_device *dev)
{
	...
	/* UVC standalone setup. */
	if (dev->run_standalone) {
		ftprintf("UVC: VIDIOC_QBUF userptr: %d\n", dev->nbufs);
		for (i = 0; i < dev->nbufs; ++i) {
			struct v4l2_buffer buf;
...
uvc_video_reqbufs_mmap(struct uvc_device *dev, int nbufs)
{
	...
	if (ret < 0) {
		if (ret == -EINVAL)
			ftprintf("UVC: does not support memory mapping\n");
		else
			ftprintf("UVC: Unable to allocate buffers: %s (%d).\n",
					strerror(errno), errno);
		goto err;
	}
	...
	if (rb.count < 2) {
		ftprintf("UVC: Insufficient buffer memory.\n");
		ret = -EINVAL;
		goto err;
	}
	...
	for (i = 0; i < rb.count; ++i) {
		...
		if (ret < 0) {
			ftprintf("UVC: VIDIOC_QUERYBUF failed for buf %d: "
				"%s (%d).\n", i, strerror(errno), errno);
			ret = -EINVAL;
			goto err_free;
		}
		...
		if (MAP_FAILED == dev->mem[i].start) {
			ftprintf("UVC: Unable to map buffer %u: %s (%d).\n", i,
				strerror(errno), errno);
			dev->mem[i].length = 0;
			ret = -EINVAL;
			goto err_free;
		...
		dev->mem[i].length = dev->mem[i].buf.length;
		ftprintf("UVC: Buffer %u mapped at address %p.\n", i,
				dev->mem[i].start);
	}

	dev->nbufs = rb.count;
	ftprintf("UVC: %u buffers allocated.\n", rb.count);
...
uvc_video_reqbufs_userptr(struct uvc_device *dev, int nbufs)
{
	...
	if (ret < 0) {
		if (ret == -EINVAL)
			ftprintf("UVC: does not support user pointer i/o\n");
		else
			ftprintf("UVC: VIDIOC_REQBUFS error %s (%d).\n",
					strerror(errno), errno);
		goto err;
	}
...
uvc_handle_streamon_event(struct uvc_device *dev)
{
	...
	if (dev->run_standalone) {
		...
		dev->is_streaming = 1;
		ftprintf("UVC Start streaming\n");
	}

	return 0;

err:
	ftprintf("Error: %s: failed to start streaming: %d\n", __func__, ret);
	return ret;
}
			   ...
			   uint8_t len, struct uvc_request_data *resp)
{
	...
	ftprintf("control request (req %02x cs %02x)\n", req, cs);
}
			     ...
			     struct uvc_request_data *resp)
{
	...
	ftprintf("streaming request (req %02x cs %02x)\n", req, cs);
...
{
	ftprintf("%s: cs=%d entity-id=%d length=%d\n", __func__,
		(int)cs, (int)entity_id, (int)data->length);
	switch (entity_id) {
	/* Processing unit 'UVC_VC_PROCESSING_UNIT'. */
	case 2:
		switch (cs) {
	...
	ftprintf("Control Request data phase (cs %02x entity %02x)\n",
			cs, entity_id);
...
uvc_events_process_data(struct uvc_device *dev, struct uvc_request_data *data)
{
	...
	case UVC_VS_PROBE_CONTROL:
		ftprintf("setting probe control, length = %d\n", data->length);
		target = &dev->probe;
		break;

	case UVC_VS_COMMIT_CONTROL:
		ftprintf("setting commit control, length = %d\n", data->length);
		target = &dev->commit;
		break;

	default:
		ftprintf("setting unknown control, length = %d\n", data->length);
	...
	case V4L2_PIX_FMT_H264:
		if (dev->imgsize == 0)
			ftprintf("WARNING: MJPEG/h.264 requested and no image loaded.\n");
...
static void dump_resp_data(struct uvc_request_data *resp)
{
	int i = 0;
	while (i < resp->length) {
		if ((i % 16) == 0)
			__ftprintf("\t");
		__ftprintf("%02x ", resp->data[i]);
		i++;
		if ((i % 8) == 0)
			__ftprintf(" ");
		if ((i % 16) == 0)
...
static const char *v4l2_event_type2str(unsigned int type)
{
	switch (type) {
...
uvc_events_process(struct uvc_device *dev)
{
	...
	if (ret < 0) {
		ftprintf("VIDIOC_DQEVENT failed: %s (%d)\n", strerror(errno),
			errno);
		return;
	}
	ftprintf("%s:VIDIOC_DQEVENT: type = %x(%s)\n", __func__,
		(unsigned int)v4l2_event.type,
		 v4l2_event_type2str(v4l2_event.type));
	...
	case UVC_EVENT_STREAMOFF:
		...
		return;
	default:
		ftprintf("Unhandled event type\n");
		return;
	}
	if (resp.length == -EAGAIN)
		return;

	ftprintf("%s: start UVCIOC_SEND_RESPONSE: len=%d \n",
		__func__, resp.length);
	dump_resp_data(&resp);
	ret = ioctl(dev->uvc_fd, UVCIOC_SEND_RESPONSE, &resp);
	if (ret < 0) {
		ftprintf("UVCIOC_S_EVENT failed: %s (%d)\n", strerror(errno),
			errno);
		return;
	}
	ftprintf("%s: End UVCIOC_SEND_RESPONSE: ret=%d \n", __func__, ret);
}
...
image_load(struct uvc_device *dev, const char *img)
{
	...
	if (fd == -1) {
		ftprintf("Unable to open MJPEG image '%s'\n", img);
		return;
	}
	...
	if (dev->imgdata == NULL) {
		ftprintf("Unable to allocate memory for MJPEG image\n");
		dev->imgsize = 0;
		return;
	}
...
usage(const char *argv0)
{
	...
	fprintf(stderr, " -a	Reply All IN direction packets\n");
...
main(int argc, char *argv[])
{
	...
	int default_format = 2;		/* V4L2_PIX_FMT_H264 */
	...
	while ((opt = getopt(argc, argv, "bdf:hi:m:n:o:r:s:t:u:v:a")) != -1) {
		switch (opt) {
		case 'b':
			bulk_mode = 1;
			break;
		...
		case 'm':
			...
			mult = atoi(optarg);
			ftprintf("Requested Mult value = %d\n", mult);
			break;
		...
		case 'n':
			...
			nbufs = atoi(optarg);
			ftprintf("Number of buffers requested = %d\n", nbufs);
			break;
		...
		case 'o':
			...
			ftprintf("UVC: IO method requested is %s\n",
		...
		case 't':
			...
			burst = atoi(optarg);
			ftprintf("Requested Burst value = %d\n", burst);
			break;
