...
#define mmiotool_help	"Usage: mmiotool [-r PHYSADDR filename] [-w PHYSADDR VALUE] [-R PHYSADDR RANGE ] [-m SLEEP] [-i PHYSADDR filename] -h \n" \
			...
			"          -R, --mread PHYSADDR RANGE filename: Read RANGE bytes starting from physical address PHYSADDR output to the filename\n" \
			"          -m, --msleep SLEEP         : Sleep for SLEEP milliseconds \n" \
			"          -i, --input filename: write filename's data to the physical address PHYSADDR\n" \
...
u_int32_t 	mmio_mrd_file(u_int32_t phys_addr, u_int32_t size,const char* filename)
{
	u_int32_t offset/*, i,value*/;
	u_int32_t *mem;
	int32_t fd;
	u_int8_t* buf;
	mem = mmio_mmap(phys_addr, PROT_READ, &offset);
	...
	if(fd >=0)
	{
		while(1) {
			mem = mmio_mmap(phys_addr, PROT_READ, &offset);
			u_int32_t write_size = page_size- offset * sizeof(u_int32_t);
			buf = (u_int8_t*) &mem[offset];
			if(size>write_size)
			{
...
u_int32_t mmio_mrd(u_int32_t phys_addr, u_int32_t size)
{
	...
	while (i<size) {
		...
		if (offset*4 == (u_int32_t)page_size) {
			munmap(mem, page_size);
			mem = mmio_mmap(phys_addr+i, PROT_READ, &offset);
		}
...
int mmio_input_fr_file(u_int32_t phys_addr, const char* filename)
{
	u_int32_t size;
	int32_t fd;
	u_int32_t offset;
	...
	DEBUGP("mmio_wr(phys_addr=0x%08x from %s\n",
		phys_addr, filename);
	...
	if(fd >=0)
	{
		...
		{
			mem = mmio_mmap(phys_addr, PROT_WRITE, &offset);
			printf("page_size=%d,offset=%d\n",page_size,offset);
			buf =(u_int8_t*) &mem[offset];
			size = read(fd,buf,page_size-offset*sizeof(u_int32_t));
			munmap(mem, page_size);
			phys_addr +=size;

		}while(size == (page_size-offset*sizeof(u_int32_t)));
		close(fd);
		DEBUGP("returning success\n");
	}
	else
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "r:R:i::w::hm:", opts, NULL);
		if (c == -1)
			break;
		...
		case 'R':
			...
			if (index < argc) {
				...
				if (index >= argc) {
					...
					mmio_mrd_file(phys_addr, val,argv[index]);
				}
			} else
		...
		case 'i':
			...
			if (index < argc) {
				phys_addr = get_physaddr(argv[index-1]);
				mmio_input_fr_file(phys_addr, argv[index]);
				printf("Wrote to  0x%08x from: %s\n", phys_addr, argv[index]);
			} else
				printf(mmiotool_help);
			break;
		case 'w':
