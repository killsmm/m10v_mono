...
static gboolean netsec_prepare (GstRTSPMedia * media, GstRTSPThread * thread);
...
static gint gst_rtsp_request_stream(T_RES_STREAM_INFOR* stream_data);
static gint set_stream_cap(GstRTSPStream *stream, const T_STREAM_INFOR* stream_infor);
#define C_ENUM(v) ((gint) v)
int gRTSP_func=0;//RTSP function on/off flag
...
gst_rtsp_media_collect_streams (GstRTSPMedia * media)
{
  ...
  if(gRTSP_func) {
	...
	T_RES_STREAM_INFOR stream_data;
	GstRTSPStream *stream;
	gint stream_nums=gst_rtsp_media_n_streams(media);
	...
	have_elem = FALSE;
  //Set Pay keyward to cap temporally. May be it should set to ipcusrc
  	GST_INFO("stream_nums:%d",stream_nums);

	if( stream_nums== 0) {
		memset(&stream_data,0,sizeof(stream_data));
		stream_data.request_id = E_NETSEC_APL_ID_GET_STREAM;
		if(gst_rtsp_request_stream(&stream_data) == 0) {
		for (i = 0; i<stream_data.stream_num; i++) {
				gchar *name;
				gchar *padname;
				gchar *pay_name;
				GstPad *src_pad;
				GstElement* pay;
				name = g_strdup_printf ("stream%d", i);
				padname = g_strdup_printf ("stream_pad%d", i);
				pay_name = g_strdup_printf ("stream_pay%d", i);
				//Create stream
				src_pad = gst_pad_new (padname, GST_PAD_SRC);
				pay = gst_element_factory_make ("ipcusrc",pay_name) ;
				stream = gst_rtsp_media_create_stream (media, pay, src_pad);
				set_stream_cap(stream, &stream_data.stream_infor[i]);
    ...
    GstPad * pad)
{
  ...
  GST_INFO("pad:%s,ghostpad:%s",gst_pad_get_name (pad),gst_pad_get_name (ghostpad));
  ...
  if (GST_PAD_IS_SRC (pad)) {
    ...
    n = priv->streams->len;
	GST_INFO ("priv->streams-len: %d", priv->streams->len);
    for (i = 0; i < n; i++) {
      GstRTSPStream *stream = g_ptr_array_index (priv->streams, i);
...
/**
 * gst_rtsp_media_find_stream_index:
 * @media: a #GstRTSPMedia
 * @control: the control of the stream
 *
 * Find a stream index in @media with @control as the control uri.
 *
 * Returns: (nullable) (transfer none): the #GstRTSPStream with
 * control uri @control or %-1 when a stream with that control did
 * not exist.
 */
gint
gst_rtsp_media_find_stream_index (const GstRTSPMedia * media, const GstRTSPStream * stream)
{
  GstRTSPMediaPrivate *priv;
  gint res;
  gint i;

  g_return_val_if_fail (GST_IS_RTSP_MEDIA (media), -1);
...
gst_rtsp_media_set_status (GstRTSPMedia * media, GstRTSPMediaStatus status)
{
  ...
  g_mutex_lock (&priv->lock);
  priv->status = status;
  GST_INFO ("setting new status to %d", status);
...
static gint gst_rtsp_get_streams(gint soc, T_RES_STREAM_INFOR* stream_data)
{
	int numrcv=0,receive_buffer_counter=0;
	char* buffer=(char*)stream_data;

	send(soc, (gchar*)(&(stream_data->request_id)), sizeof(stream_data->request_id),0);
//	do {
		numrcv = recv(soc, 
					&buffer[receive_buffer_counter], 
					sizeof(T_RES_STREAM_INFOR)-receive_buffer_counter, 0); 
		receive_buffer_counter +=numrcv;
//	}while(numrcv>0 && receive_buffer_counter<sizeof(T_RES_STREAM_INFOR));

	return receive_buffer_counter;
}
static gint gst_rtsp_request_stream(T_RES_STREAM_INFOR* stream_data)
{
	...
	gint soc = socket( PF_UNIX, SOCK_STREAM, 0 );
	if(soc < 0 ) {
		GST_ERROR ("Can't open stream socket");
		ret = -1;
	}
	else {
		...
		if ( connect( soc, ( struct sockaddr * )&saddr, ( socklen_t )sizeof( saddr ) ) < 0 ) {
			GST_ERROR ("Can't connect stream socket");
			ret = -2;
		}
		else {
			int read_size=gst_rtsp_get_streams(soc,stream_data);

			GST_INFO ("read_size:%d,request_id:%u,stream_num:%u,stream_id:%u,clock_rate:%u,media_type:%u,payload_id:%u,frame_rate:%u,encode_type:%u",
				read_size,
				stream_data->request_id,
				stream_data->stream_num,
				stream_data->stream_infor[0].stream_id,
				stream_data->stream_infor[0].clock_rate,
				stream_data->stream_infor[0].media_type,
				stream_data->stream_infor[0].payload_id,
				stream_data->stream_infor[0].frame_rate,
				stream_data->stream_infor[0].encode_type
				);
			if(stream_data->stream_num >=D_MAX_STREAM_NUM){
				ret = -3;
			}
...
static gint set_stream_cap(GstRTSPStream *stream, const T_STREAM_INFOR* stream_infor)
{
	...
	gchar profile_level_id[20];
	gchar width[10];
	gchar height[10];
	gchar depth[5];

	const gchar* sampleing=NULL;
	const gchar* colorimetry=NULL;

	switch(stream_infor->media_type){
	case D_MEDIA_TYPE_VIDEO:
		media_name = "video";
		break;
	case D_MEDIA_TYPE_AUDIO:
		media_name = "audio";
		break;
	default:
		media_name = "metadata";
	...
	switch(stream_infor->encode_type){
	...
	case E_ENCODE_TYPE_ACC:
		encoding_name = "MP4A-LATM";
		break;
	case E_ENCODE_TYPE_META:
		encoding_name = "Metadata";
		break;
	default:
		encoding_name="RAW";
		switch(stream_infor->sampleing){
		case E_SAMPLING_RGB:
			sampleing ="RGB";
			break;
		case E_SAMPLING_YCC422:
			sampleing ="YCbCr-4:2:2";
			break;
		case E_SAMPLING_YCC420:
			sampleing ="YCbCr-4:2:0";
			break;
		default:
			sampleing ="YCbCr-4:4:4";
			break;
		}
		break;
	}
	sprintf(profile_level_id,"%u",stream_infor->profile_level_id);
	switch(stream_infor->media_type){
	case D_MEDIA_TYPE_VIDEO:
	case D_MEDIA_TYPE_AUDIO:
		switch(stream_infor->encode_type){
		case E_ENCODE_TYPE_YUV:
			sprintf(width,"%u",stream_infor->width);
			sprintf(height,"%u",stream_infor->height);
			sprintf(depth,"%u",stream_infor->depth);
			switch(stream_infor->colorimetry){
			case E_COLORIMETRY_BT709:
				colorimetry ="BT.709-2";
				break;
			case E_COLORIMETRY_BT601:
				colorimetry ="BT.601";
				break;
			default:
			...
			caps =
				...
				"media",G_TYPE_STRING,media_name,
				"encoding-name",G_TYPE_STRING,encoding_name,
				"payload",G_TYPE_INT,stream_infor->payload_id,
				"sampling",G_TYPE_STRING,sampleing,
				"width", G_TYPE_STRING, width,
				"height", G_TYPE_STRING, height,
				"depth",G_TYPE_STRING,depth,
				"colorimetry",G_TYPE_STRING,colorimetry,
				"chroma-position",G_TYPE_STRING,"1",
				"clock-rate",G_TYPE_INT,stream_infor->clock_rate,
				"framerate",G_TYPE_INT, stream_infor->frame_rate,
				"packetization-mode",G_TYPE_STRING,"1",
				"profile-level-id",G_TYPE_STRING,profile_level_id,
				NULL); 
			break;
		case E_ENCODE_TYPE_MJPEG:
				if(stream_infor->width >2048 || stream_infor->height >2048) {
					gchar dimensions[20];
					sprintf(dimensions,"%u,%u",
						stream_infor->width,
						stream_infor->height);
					//If size are greater than 2048, set size at SDP
					caps=
					gst_caps_new_simple ("application/x-rtp", 
					"media",G_TYPE_STRING,media_name,
					"encoding-name",G_TYPE_STRING,encoding_name,
					...
					"profile-level-id",G_TYPE_STRING,profile_level_id,
					"framerate",G_TYPE_INT, stream_infor->frame_rate,
					"x-dimensions", G_TYPE_STRING, dimensions,
					NULL); 
				}
				else{
					caps=
	...
	gst_rtsp_stream_set_caps(caps,stream);

	gst_rtsp_stream_set_profiles(stream,GST_RTSP_PROFILE_AVP);

	return ret;
}

static gboolean
netsec_prepare (GstRTSPMedia * media, GstRTSPThread * thread)
{
  GstRTSPMediaPrivate *priv;
  GstRTSPMediaClass *klass;
  GstBus *bus;
...
static int releasa_stream(uint stream_id)
{
	...
	if(soc < 0 ) {
		...
		if ( connect( soc, ( struct sockaddr * )&saddr, ( socklen_t )sizeof( saddr ) ) < 0 ) {
			...
			T_REMOVE_STREAM remove_stream;
			T_NETSC_APL_RESULT remove_stream_result;

			remove_stream.request_id = E_NETSEC_APL_ID_RMV_STREAM;
			remove_stream.stream_id = stream_id;

			send(soc, (gchar*)(&remove_stream), sizeof(remove_stream),0);
			recv(soc, 
					&remove_stream_result, 
					sizeof(remove_stream_result), 0); 
			ret= remove_stream_result.result;
