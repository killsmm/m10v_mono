...
static void
make_media_netsec (GstSDPMessage * sdp, GstSDPInfo * info,
    GstRTSPStream * stream, GstCaps * caps, GstRTSPProfile profile)
{
  ...
  GstStructure *s;
  gint caps_pt, caps_rate,caps_width,caps_height;

  gst_sdp_media_new (&smedia);
   GST_INFO ("caps:%p profile:%d",caps,profile);
  ...
  if (s != NULL) {
	...
	GST_INFO ("encode_name:%s",encode_name);
	if (strcmp(encode_name,"JPEG")==0 || strcmp(encode_name,"RAW")==0) {
		gint cap_frame_rate;
		gst_structure_get_int (s, "framerate", &cap_frame_rate);
		tmp =
		  g_strdup_printf ("%d", cap_frame_rate);
		gst_sdp_media_add_attribute (smedia, "framerate", tmp);
		g_free (tmp);

	}
  }
  else
	GST_INFO ("caps is NULL");
    ...
    GstRTSPStream * stream)
{
  ...
  while (profiles >= mask) {
    ...
    if (prof) {
		if(gRTSP_func >= 1) {
		//RTSP valid
			make_media_netsec(sdp, info, stream, caps, prof);	
		} else {
