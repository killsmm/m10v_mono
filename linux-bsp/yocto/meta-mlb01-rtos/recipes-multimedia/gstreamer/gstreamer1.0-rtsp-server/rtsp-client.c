 ...
 *     Author: Sebastian Dr旦ge <sebastian@centricular.com>
 *
...
#define D_SERVER_PORT_NO_RANGE	(64)
#define D_PORT_START	(50000)
typedef struct{
	guint using_flag;
	gchar session_id[128];
}T_PORT_MNG;
...
struct _GstRTSPClientPrivate
{
  ...
  T_PORT_MNG port_mng_tbl[D_SERVER_PORT_NO_RANGE];
};
...
static gboolean
netsec_configure_client_transport (GstRTSPClient * client,
...
static gint release_server_port( GstRTSPContext * ctx);
static gint gst_rtsp_stream_play();
...
gst_rtsp_client_class_init (GstRTSPClientClass * klass)
{
  ...
  if (gRTSP_func) {
	//RTSP valid
	  klass->configure_client_transport = netsec_configure_client_transport;
  } else {
...
gst_rtsp_client_init (GstRTSPClient * client)
{
  ...
  if(gRTSP_func) {
	//RTSP valid
	memset(priv->port_mng_tbl,0,sizeof(priv->port_mng_tbl));
  }
}
    ...
    GstRTSPContext * ctx, GstRTSPTransport * ct)
{
  ...
  GST_INFO ("default_configure_client_transport");
...
static 
gint gst_rtsp_open_socket_apl(void)
{
...
static gint get_server_port( GstRTSPContext * ctx, guint src_port[2])
{
	gint i;
	gint ret=-1;
	GstRTSPClientPrivate *priv = ctx->client->priv;

	const gchar *session_id = gst_rtsp_session_get_sessionid(ctx->session);
	if(session_id) {

		for(i=0;i<D_SERVER_PORT_NO_RANGE;i+=2){
			if(priv->port_mng_tbl[i].using_flag == 0){
				src_port[0]=D_PORT_START+i;
				src_port[1]=D_PORT_START+i+1;
				priv->port_mng_tbl[i].using_flag=
				priv->port_mng_tbl[i+1].using_flag=1;
				strncpy(priv->port_mng_tbl[i].session_id, session_id,sizeof(priv->port_mng_tbl[i].session_id));
				ret=0;
				break;
			}
		}
...
static gint release_server_port( GstRTSPContext * ctx)
{
	...
	if(session_id) {
		...
		for(i=0;i<D_SERVER_PORT_NO_RANGE;i+=2){
			if(strcmp(priv->port_mng_tbl[i].session_id, session_id) == 0){
				priv->port_mng_tbl[i].using_flag=
				priv->port_mng_tbl[i+1].using_flag=0;
				priv->port_mng_tbl[i].session_id[0] = 0;
				ret=0;
			}
		}
	}
...
static gint gst_rtsp_stream_play(void)
{
	gint soc = gst_rtsp_open_socket_apl();
	gint ret = 0;
	if (soc < 0) {
		GST_ERROR ("Can't open stream socket");
		ret = -1;
		...
		guint request_id = E_NETSEC_APL_ID_PLAY_STREAM;
		send(soc, (gchar*)(&request_id), sizeof(request_id),0);
		close( soc );
	}
	return ret;
}
static gint gst_rtsp_stream_netsec_udp_sockets(GstRTSPContext * ctx,
	  GstRTSPTransport * ct)
{
	gint soc = gst_rtsp_open_socket_apl();
	gint ret = 0;
	if (soc < 0) {
		...
		T_ADD_STREAM add_stream;
		memset(&add_stream,0,sizeof(add_stream));
		add_stream.stream_id = gst_rtsp_media_find_stream_index( ctx->media,  ctx->stream);
		if(add_stream.stream_id >=0) {
			if(get_server_port(ctx,add_stream.src_port) == 0) {
				gint recv_num,receive_buffer_counter;
				T_NETSC_APL_RESULT receive_result;
				char* receive_buffer =(char*)(&receive_result);

				add_stream.des_port[0] = ct->client_port.min;
				add_stream.des_port[1] = ct->client_port.max;
				strncpy(add_stream.des_ip, ct->destination,strlen(ct->destination));
				ct->server_port.min = add_stream.src_port[0];
				ct->server_port.max = add_stream.src_port[1];
				add_stream.request_id = E_NETSEC_APL_ID_ADD_STREAM;

				send(soc, (gchar*)(&add_stream), sizeof(add_stream),0);
...
//				do {
					recv_num = recv(soc,
						&receive_buffer[receive_buffer_counter],
						sizeof(receive_result)
						,0);
						receive_buffer_counter +=recv_num;
//				}while(recv_num>0 && receive_buffer_counter<sizeof(receive_result));

				ret =receive_result.result;
			}
		}
		else{
			GST_ERROR ("Illegal stream");
			ret = -2;
		}
		close( soc );
	}
...
//Set session ID, Port number, etc. information to extern process
//so that they start delivery stream .
static gboolean
netsec_configure_client_transport (GstRTSPClient * client,
    GstRTSPContext * ctx, GstRTSPTransport * ct)
{
	...
	//Only think about UDP single cast
    GstRTSPUrl *url =gst_rtsp_connection_get_url (priv->connection);
	gboolean ret = TRUE;

	GST_INFO ("netsec_configure_client_transport");
	
	g_free (ct->destination);
	ct->destination = g_strdup (url->host);
    if (gst_rtsp_stream_netsec_udp_sockets(ctx,ct) == 0) {
		GST_INFO ("port[%d,%d],server_port[%d,%d],client_port[%d,%d],source:%s,destination:%s",
			ct->port.min,ct->port.max,
			ct->server_port.min,ct->server_port.max,
			ct->client_port.min,ct->client_port.max,
			ct->source,
			ct->destination);
	}
	else {
...
handle_setup_request (GstRTSPClient * client, GstRTSPContext * ctx)
{
  ...
  if (path[matched] == '\0') {
	...
	GST_INFO("path:%s,matched:%d,control:%s,stream:%p",path,matched,control,stream);
  }
  ...
  GST_INFO ("port[%d,%d],server_port[%d,%d],client_port[%d,%d],source:%s,destination:%s,ssrc:%u,session:%s,trans_str:%s",
		st->port.min,st->port.max,
		st->server_port.min,st->server_port.max,
		st->client_port.min,st->client_port.max,
		st->source,
		st->destination,st->ssrc,gst_rtsp_session_get_sessionid(ctx->session),
		trans_str);
