...
main (int argc, char *argv[])
{
  ...
  if( argc < 2 )
  {
    ...
    printf("  sni-gstrtsp <eth0 or mlan0> <\"ipcusrc ! h264parse !rtph264pay name=pay0 pt=96\">\n");
    return -1;
  }
  ...
  gst_rtsp_media_factory_set_launch (factory, "( "
      "ipcusrc ! h264parse !rtph264pay name=pay0 pt=96"
       ")");
  #else
...
#ifdef WITH_TLS
  g_print ("stream ready at rtsps://%s:%s/video\n",ip,port);
#else
  g_print ("stream ready at rtsp://%s:%s/video\n",ip,port);
#endif
