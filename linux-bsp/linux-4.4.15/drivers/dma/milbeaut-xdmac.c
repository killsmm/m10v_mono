...
{
	u32 val;
	//printk("xdmac %s(), mc->reg_ch_base:%08x\n", __func__, mc->reg_ch_base);
	/* Setup the channel */
	val = md->len - 1;
				    ...
				    int chan_id)
{
	...
	//printk("xdmac %s, mdev->reg_base:%08x, finished\n", __func__, mdev->reg_base);
	return 0;
}
...
static int milbeaut_xdmac_probe(struct platform_device *pdev)
{
	...
	ddev->directions = BIT(DMA_MEM_TO_MEM)| BIT(DMA_DEV_TO_MEM) | BIT(DMA_MEM_TO_DEV);
