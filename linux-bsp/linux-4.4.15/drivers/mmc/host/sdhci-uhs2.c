...
#define FD_ONLY  /* Restriction : F_SDH40 cannot support HD312 mode */
...
static int sdhci_uhs2_prepare_exit_dormant(struct sdhci_host *host)
{
	const struct sdhci_ops *sdhci_ops = host->ops;

	if (sdhci_ops->exit_dormant_quirk)
		sdhci_ops->exit_dormant_quirk(host);
...
int sdhci_uhs2_do_set_reg(struct sdhci_host *host, enum uhs2_act act)
{
	...
	case CHECK_DORMANT:
		...
		break;
	case PREPARE_EXIT_DORMANT:
		err = sdhci_uhs2_prepare_exit_dormant(host);
		break;
	default:
