...
#define ERS01(adr)         ((void __iomem *)((adr)+0x0304))
...
#define  HRS45_CT_CT_40    (0x40 << 8)
#define  HRS45_CT_CT_44    (0x44 << 8)
					...
					irq_handler_t fn0, irq_handler_t fn1)
{
	...
	val |= HRS45_CT_CT_40 | HRS45_RCLK2EN_TST | HRS45_CT_NRST;
...
static int sdhci_f_sdh40_uhs1_to_uhs2_setting(struct sdhci_host *host)
{
	struct f_sdhost_priv *priv = sdhci_priv(host);
	struct mmc_host *mmc = host->mmc;
				...
				irq_handler_t fn0, irq_handler_t fn1)
{
	...
	sdhci_f_sdh40_uhs1_to_uhs2_setting(host);
...
void sdhci_f_sdh40_exit_dormant_quirk(struct sdhci_host *host)
{
	...
	/* To avoid the H/W bug of hung up at exit-dormant,
	 * apply PHY reset before clock enable
	 */
	val = HRS45_RCLK2EN_TST;
	if (readl(ERS01(priv->ioaddr_sd40_sn)) & ERS01_SPDR) {
		val |= HRS45_CT_CT_44;
	} else {
		val |= HRS45_CT_CT_40;
	}
				...
				const char *buf, size_t count)
{
	...
	if (enable) {
		...
		devm_gpio_free(dev, priv->reset_gpio);
