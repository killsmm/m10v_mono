...
static void __iomem *g_hdmi_dcore_reg_base=0;
static void __iomem *g_lcd_dcore_reg_base=0;
...
static int check_size(struct fb_info *info, struct fb_disp *user_disp)
{
	...
	if ((check_pich_offset(gr_rzsl,
		...
		/* Check input size */
		if (fb_mlb_par->block_no == 1 || fb_mlb_par->block_no == 3) {
	...
	int block_no)
{
	...
	if (block_no == BLOCK_HDMI0 || block_no == BLOCK_HDMI1) {
			...
			u32 one_pixel_byes)
{
	...
	//printk("%s, %d, MAX_DRAW_AREA_NUM:%d\n", __func__, __LINE__, MAX_DRAW_AREA_NUM);
	for (i = 0; i < MAX_DRAW_AREA_NUM; i++) {
		if (disp->enable_show_area[i] == 1) {
			//printk("%s, %d, i:%d, disp->valid_buffer_number:%d\n", __func__, __LINE__, i, disp->valid_buffer_number);
					...
					u32 buf_size)
{
	...
	if (child) {
		...
		disp->drawing_buffer_top_address_size[0] = buf_addr,
		disp->drawing_buffer_top_address_size[1] = buf_size,
		pr_info("fb[%d] : %s :buf=0x%08x, size=0x%08x\n",
				fb_mlb_par->block_no, pattern_name,
				buf_addr, buf_size);
		...
		struct fb_info *info)
{
	...
	//TBD, please check byte_bit_mask setting of HDMI1 and LCD1
	if (fb_mlb_par->block_no == BLOCK_HDMI0 || fb_mlb_par->block_no == BLOCK_HDMI1) {
		byte_bit_mask = 0x0F;/* Must be over 16 bytes */
	} else {
		byte_bit_mask = 0x07;/* Must be over 4 bytes */
	}
...
static int get_shared_mem_info(char *pattern, u32 *val)
{
	struct device_node *shmem_node;
	u32 addr;
	void __iomem *ipcu_buff_mem;

	shmem_node = of_find_compatible_node(NULL, NULL,
	...
	if (of_property_read_u32(shmem_node, pattern, &addr)) {
		pr_err("get %s failed\n", pattern);
		return -EINVAL;
	}

	ipcu_buff_mem = ioremap(addr, sizeof(u32));
	if (!ipcu_buff_mem) {
		pr_err("get ipcu-buffer-mem ioremap failed\n");
		return -EINVAL;
	}

	*val = ioread32(ipcu_buff_mem);
...
static int milb_disp_probe(struct platform_device *pdev)
{
	...
	u32 temp;
	struct fb_mlb_priv *fb_mlb_par;
	u32 buf_addr[4], buf_size[4];

	if (get_shared_mem_info("lcd-osd-gr0-frame-addr",
				&buf_addr[BLOCK_LCD0]) < 0) {
		pr_err("get lcd-osd-gr0-frame-addr failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("lcd-osd-gr0-frame-size",
				&buf_size[BLOCK_LCD0]) < 0) {
		pr_err("get lcd-osd-gr0-frame-size failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("hdmi-osd-gr0-frame-addr",
				&buf_addr[BLOCK_HDMI0]) < 0) {
		pr_err("get hdmi-osd-gr0-frame-addr failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("hdmi-osd-gr0-frame-size",
				&buf_size[BLOCK_HDMI0]) < 0) {
		pr_err("get hdmi-osd-gr0-frame-size failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("lcd-osd-gr1-frame-addr",
				&buf_addr[BLOCK_LCD1]) < 0) {
		pr_err("get lcd-osd-gr1-frame-addr failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("lcd-osd-gr1-frame-size",
				&buf_size[BLOCK_LCD1]) < 0) {
		pr_err("get lcd-osd-gr1-frame-size failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("hdmi-osd-gr1-frame-addr",
				&buf_addr[BLOCK_HDMI1]) < 0) {
		pr_err("get hdmi-osd-gr1-frame-addr failed\n");
		return -EINVAL;
	}
	if (get_shared_mem_info("hdmi-osd-gr1-frame-size",
				&buf_size[BLOCK_HDMI1]) < 0) {
		pr_err("get hdmi-osd-gr1-frame-size failed\n");
		return -EINVAL;
	}
	printk(" buf_addr[BLOCK_HDMI0]:%x\n", buf_addr[BLOCK_HDMI0]);
	printk(" buf_size[BLOCK_HDMI0]:%x\n", buf_size[BLOCK_HDMI0]);
	printk(" buf_addr[BLOCK_HDMI1]:%x\n", buf_addr[BLOCK_HDMI1]);
	printk(" buf_addr[BLOCK_HDMI1]:%x\n", buf_size[BLOCK_HDMI1]);
	...
	printk("%s, fb_mlb_par->gr_no:%d, index:%d\n", __func__, fb_mlb_par->gr_no, index);

	/* Get DCORE base */
	/* Get DCORE's IO address */
#if 0
	if(g_dcore_reg_base==0){
	//	printk("%s, ioremap\n", __func__);
		res_dcore = platform_get_resource_byname(pdev,
			IORESOURCE_MEM, "dcore");
		g_dcore_reg_base = fb_mlb_par->dcore_reg_base =
			devm_ioremap_resource(dev, res_dcore);
	}else{
	//	printk("%s assign dcore_reg_base\n", __func__);
		fb_mlb_par->dcore_reg_base = g_dcore_reg_base;
	}
#else
	/*FIXME, there are 4 times initial for lcd-gr0, lcd-gr1, hdmi-gr0 and hdmi-gr1.
	the dcore address is 0x28901000 for hdmi and 0x28905000 is for lcd.
	that is, gr1 will fail due to gr0 already use the address
	we use a global variant to keep gr0 address and assign it go gr1*/
	res_dcore = platform_get_resource_byname(pdev,
		IORESOURCE_MEM, "dcore");

	if(res_dcore->start==0x28905000){
		if(g_lcd_dcore_reg_base==0){
			g_lcd_dcore_reg_base = fb_mlb_par->dcore_reg_base =
				devm_ioremap_resource(dev, res_dcore);
		}else{
			fb_mlb_par->dcore_reg_base = g_lcd_dcore_reg_base;
		}
	}else if(res_dcore->start==0x28901000){
		if(g_hdmi_dcore_reg_base==0){
			g_hdmi_dcore_reg_base = fb_mlb_par->dcore_reg_base =
				devm_ioremap_resource(dev, res_dcore);
		}else{
			fb_mlb_par->dcore_reg_base = g_hdmi_dcore_reg_base;
		}
	}else{
		printk("%s, dcore address 0x%x can't be recognized\n", __func__, res_dcore->start);
	}
	...
	printk("%s, fb_mlb_par->block_no:%d\n", __func__, fb_mlb_par->block_no);
...
static int milb_disp_suspend(struct device *dev)
{
	...
	mutex_lock(&gr_ipcu.mlock);
	if (fb_mlb_par->block_no == BLOCK_LCD0 || fb_mlb_par->block_no == BLOCK_LCD1) {
...
#else
static int __init milb_disp_fb_init(void)
{
        return platform_driver_register(&milb_disp_fb_driver);
}
device_initcall_sync(milb_disp_fb_init);
static void __exit milb_disp_fb_exit(void)
{
        return platform_driver_unregister(&milb_disp_fb_driver);
}
module_exit(milb_disp_fb_exit);
