			   ...
			   u8 bit_length, u32 value)
{
	...
	reg = readl_relaxed(base + EPCMD(id));

	reg &= ~((0x1f << 18) | (0x1ff << 0)); /* clear volatile bits */

	ctrl = readl_relaxed(base + EPCTRL(id));
	reg |= (VALUE(ctrl, 22, 1) << 16); /* nacken */
...
#else
static inline void hdc_writel(u32 val, volatile void __iomem *add)
{
	writel(val, add);
}

static inline u32 hdc_readl(volatile void __iomem *add)
{
	return readl(add);
}
			   ...
			   u32 bit_length, u32 value)
{
	...
	if (offset == INTS) {
		...
		if(reg!=300){
			if (start_bit == DMA_INT(0))
				reg |= 0x218;
			else if (start_bit == DMA_INT(1))
				reg |= 0x118;
			else if (start_bit == CMD_INT)
				reg |= 0x308;
			else if (start_bit == PHY_ERR_INT)
				reg |= 0x310;
			else
				reg |= 0x318;
			//printk("INTS(%d): set 0x%x\n", start_bit, reg);
			//printk("%s, reg 4: %x\n", __func__, reg);
		}
	}else{
		reg_oris = reg = readl_relaxed(udc->reg_base + offset);
		reg &= ~(MASK(bit_length) << start_bit);
		reg |= ((value & MASK(bit_length)) << start_bit);
...
static void sn_xfer_ep0_in(struct usb_request *r)
{
	...
	if (bytes == ep->ep_usb.maxpacket)
		hdc_write_ep(udc, 0, NACK_INTEN, 1, 1);
	else
		hdc_write_ep(udc, 0, NACK_INTEN, 1, 0);
...
static void sn_xfer_ep0_out(struct usb_request *r)
{
	...
	if (r->actual == r->length) {
		...
		hdc_write_ep(udc, 0, NACK_INTEN, 1, 1);
		hdc_write_ep(udc, 0, RDYO_EMPTY_INTEN, 1, 0);
	}
}
			   ...
			   gfp_t gfp_flags)
{
	...
	if( usb_req->buf == 0 && usb_req->dma != 0 ){
		ret = 0;
	}else{
		if (need_dma(req))
...
static void sn_udc_ep0_data(struct sn_udc *udc)
{
	...
	if (!udc->ep0_active) {
		hdc_write_ep(udc, 0, NACK_INT_CLR, 1, 1);
	...
	if (VALUE(ctrl, RDYI_RDY_INT, 1) && VALUE(ctrl, READY_INTEN, 1)) {
		...
		sn_xfer_ep0_out(r);
	} else if (VALUE(ctrl, NACK_INT, 1)
			 && VALUE(ctrl, EPCTRL_NACK_INTEN, 1)) {
		hdc_write_ep(udc, 0, NACK_INT_CLR, 1, 1);
		...
		if (!(udc->setup.bRequestType & USB_DIR_IN) ||
			r->actual < r->length || udc->ep0_active->zlp)
			sn_xfer_ep0_in(r);
