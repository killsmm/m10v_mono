...
static int __uvcg_iter_bm_controls(struct f_uvc_opts *opts,
			const char *page, size_t len, int count, __u8 *bmbuf)
{
	char buf[4];
	const char *pg = page;
	int i, cnt = 0, ret;
	...
	while (cnt < count && pg - page < len) {
		i = 0;
		while (i < sizeof(buf) && *pg != '\0' && *pg != '\n')
			buf[i++] = *pg++;
		if (i == sizeof(buf)) {
			ret = -EINVAL;
			break;
	...
	struct config_item *item, const char *page, size_t len)
{
	...
	ret = __uvcg_iter_bm_controls(opts, page, len,
				pd->bControlSize, pd->bmControls);
	if (ret >= 0)
		ret = len;
...
static ssize_t uvcg_default_extension_guid_extension_code_show(
	struct config_item *item, char *page)
{
	struct uvcg_default_extension *dp = to_uvcg_default_extension(item);
	struct f_uvc_opts *opts;
	...
	result = sprintf(page,
			...
			ed->guidExtensionCode[3],
			ed->guidExtensionCode[2],
			ed->guidExtensionCode[1],
			ed->guidExtensionCode[0],
			ed->guidExtensionCode[5],
			ed->guidExtensionCode[4],
			ed->guidExtensionCode[7],
			ed->guidExtensionCode[6],
			ed->guidExtensionCode[8],
			ed->guidExtensionCode[9],
			ed->guidExtensionCode[10],
			ed->guidExtensionCode[11],
			ed->guidExtensionCode[12],
			ed->guidExtensionCode[13],
			ed->guidExtensionCode[14],
			ed->guidExtensionCode[15]);
	mutex_unlock(&opts->lock);
...
UVC_ATTR_RO(uvcg_default_extension_, guid_extension_code, guidExtensionCode);

static ssize_t uvcg_default_extension_ba_source_id_show(
	struct config_item *item, char *page)
{
	struct uvcg_default_extension *dp = to_uvcg_default_extension(item);
	struct f_uvc_opts *opts;
	...
	mutex_lock(&opts->lock);
	for (result = 0, i = 0; i < ed->bNrInPins; ++i) {
		result += sprintf(pg, "%d\n", ed->baSourceID[i]);
		pg = page + result;
	}
	mutex_unlock(&opts->lock);
...
UVC_ATTR_RO(uvcg_default_extension_, ba_source_id, baSourceID);
...
static struct configfs_attribute *uvcg_default_extension_attrs[] = {
	...
	&uvcg_default_extension_attr_guid_extension_code,
	&uvcg_default_extension_attr_ba_source_id,
...
static struct config_group *uvcg_extension_default_groups[] = {
	&uvcg_default_extension.group,
	NULL,
};
...
struct uvcg_frame {
	...
	struct {
		...
		u16	w_width;
		u16	w_height;
		u16	w_sample_aspect_ratio_width;
		u16	w_sample_aspect_ratio_height;
		u16	w_profile;
		u8	b_level_idc;
		u16	w_constrained_toolset;
		u32	bm_supported_usages;
		u16	bm_capabilities;
		u32	bm_svc_capabilities;
		u32	bm_mvcc_capabilities;
		u32	dw_min_bit_rate;
		u32	dw_max_bit_rate;
		u32	dw_default_frame_interval;
		u8	b_num_frame_intervals;
	} __attribute__((packed)) frame_h264;
#endif
...
#if 1
#define UVCG_FRAME_H264_ATTR(cname, aname, to_cpu_endian, to_little_endian, bits) \
static ssize_t uvcg_frame_h264_##cname##_show(struct config_item *item, char *page)\
{									\
	struct uvcg_frame *f = to_uvcg_frame(item);			\
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
	...
	opts = to_f_uvc_opts(opts_item);				\
									\
	mutex_lock(&opts->lock);					\
	result = sprintf(page, "%d\n", to_cpu_endian(f->frame_h264.cname));	\
	mutex_unlock(&opts->lock);					\
									\
	mutex_unlock(su_mutex);						\
	return result;							\
									...
									\
static ssize_t  uvcg_frame_h264_##cname##_store(struct config_item *item,	\
					   const char *page, size_t len)\
{									\
	struct uvcg_frame *f = to_uvcg_frame(item);			\
	struct f_uvc_opts *opts;					\
	...
	if (fmt->linked || opts->refcnt) {				\
		...
		goto end;						\
									...
									\
	f->frame_h264.cname = to_little_endian(num);			\
	ret = len;							\
end:									\
	mutex_unlock(&opts->lock);					\
	mutex_unlock(su_mutex);						\
	return ret;							\
									...
									\
UVC_ATTR(uvcg_frame_h264_, cname, aname);

#define noop_conversion(x) (x)

UVCG_FRAME_H264_ATTR(bm_capabilities, bmCapabilities, le16_to_cpu,
		cpu_to_le16, 16);
UVCG_FRAME_H264_ATTR(b_frame_index, bFrameIndex, noop_conversion,
		noop_conversion, 8);
UVCG_FRAME_H264_ATTR(w_width, wWidth, le16_to_cpu, cpu_to_le16, 16);
UVCG_FRAME_H264_ATTR(w_height, wHeight, le16_to_cpu, cpu_to_le16, 16);
UVCG_FRAME_H264_ATTR(dw_min_bit_rate, dwMinBitRate, le32_to_cpu, cpu_to_le32, 32);
UVCG_FRAME_H264_ATTR(dw_max_bit_rate, dwMaxBitRate, le32_to_cpu, cpu_to_le32, 32);
UVCG_FRAME_H264_ATTR(dw_default_frame_interval, dwDefaultFrameInterval,
		le32_to_cpu, cpu_to_le32, 32);

#undef noop_conversion

#undef UVCG_FRAME_H264_ATTR

static ssize_t uvcg_frame_h264_dw_frame_interval_show(struct config_item *item,
						 char *page)
{
	struct uvcg_frame *frm = to_uvcg_frame(item);
	struct f_uvc_opts *opts;
	...
	mutex_lock(&opts->lock);
	for (result = 0, i = 0; i < frm->frame_h264.b_num_frame_intervals; ++i) {
		result += sprintf(pg, "%d\n",
				  le32_to_cpu(frm->dw_frame_interval[i]));
		pg = page + result;
	}
...
static ssize_t uvcg_frame_h264_dw_frame_interval_store(struct config_item *item,
						  const char *page, size_t len)
{
	struct uvcg_frame *ch = to_uvcg_frame(item);
	struct f_uvc_opts *opts;
	...
	kfree(ch->dw_frame_interval);
	ch->dw_frame_interval = frm_intrv;
	ch->frame_h264.b_num_frame_intervals = n;
	ret = len;
...
UVC_ATTR(uvcg_frame_h264_, dw_frame_interval, dwFrameInterval);

static struct configfs_attribute *uvcg_frame_h264_attrs[] = {
	&uvcg_frame_h264_attr_b_frame_index,
	&uvcg_frame_h264_attr_w_width,
	&uvcg_frame_h264_attr_w_height,
//	&uvcg_frame_h264_attr_w_sample_aspect_ratio_width,
//	&uvcg_frame_h264_attr_w_sample_aspect_ratio_height,
//	&uvcg_frame_h264_attr_w_profile,
//	&uvcg_frame_h264_attr_b_level_idc,
//	&uvcg_frame_h264_attr_w_constrained_toolset,
//	&uvcg_frame_h264_attr_bm_supported_usages,
	&uvcg_frame_h264_attr_bm_capabilities,
//	&uvcg_frame_h264_attr_bm_svc_capabilities,
//	&uvcg_frame_h264_attr_bm_mvcc_capabilities,
	&uvcg_frame_h264_attr_dw_min_bit_rate,
	&uvcg_frame_h264_attr_dw_max_bit_rate,
	&uvcg_frame_h264_attr_dw_default_frame_interval,
//	&uvcg_frame_h264_attr_b_num_frame_intervals,
	&uvcg_frame_h264_attr_dw_frame_interval,
	NULL,
};

static struct config_item_type uvcg_frame_h264_type = {
	.ct_attrs	= uvcg_frame_h264_attrs,
	.ct_owner	= THIS_MODULE,
};

static struct config_item *uvcg_frame_h264_make(struct config_group *group,
					   const char *name)
{
	struct uvcg_frame *h;
	struct uvcg_format *fmt;
	...
	h->frame_h264.b_descriptor_type			= USB_DT_CS_INTERFACE;
	h->frame_h264.b_frame_index			= 1;
	h->frame_h264.w_width				= cpu_to_le16(1920);
	h->frame_h264.w_height				= cpu_to_le16(1080);
	h->frame_h264.w_sample_aspect_ratio_width	= cpu_to_le16(1);
	h->frame_h264.w_sample_aspect_ratio_height	= cpu_to_le16(1);
	h->frame_h264.w_profile				= cpu_to_le16(0x6400);
	h->frame_h264.b_level_idc			= 0x28;
	h->frame_h264.bm_supported_usages		= cpu_to_le32(0x00020000);
	h->frame_h264.bm_capabilities			= cpu_to_le16(0x0026);
	h->frame_h264.dw_min_bit_rate			= cpu_to_le32(8000000);
	h->frame_h264.dw_max_bit_rate			= cpu_to_le32(8000000);
	h->frame_h264.dw_default_frame_interval		= cpu_to_le32(333333);
	...
	mutex_lock(&opts->lock);
	fmt = to_uvcg_format(&group->cg_item);
	h->frame_h264.b_descriptor_subtype = UVC_VS_FRAME_H264;
	h->fmt_type = UVCG_H264;
	++fmt->num_frames;
	mutex_unlock(&opts->lock);

	config_item_init_type_name(&h->item, name, &uvcg_frame_h264_type);

	return &h->item;
}

static void uvcg_frame_h264_drop(struct config_group *group, struct config_item *item)
{
	struct uvcg_frame *h = to_uvcg_frame(item);
	struct uvcg_format *fmt;
	struct f_uvc_opts *opts;
...
static struct configfs_group_operations uvcg_h264_group_ops = {
	.make_item		= uvcg_frame_h264_make,
	.drop_item		= uvcg_frame_h264_drop,
};
...
UVCG_H264_ATTR(b_default_frame_index, bDefaultFrameIndex,
		       identity_conv);
////UVCG_H264_ATTR_RO(bm_interface_flags, bmInterfaceFlags, identity_conv);
						   ...
						   const char *name)
{
	...
	h->desc.bLength			= UVC15_DT_FORMAT_H264_SIZE;
	h->desc.bDescriptorType		= USB_DT_CS_INTERFACE;
	h->desc.bDescriptorSubType	= UVC_VS_FORMAT_H264;
	h->desc.bDefaultFrameIndex	= 1;
	h->desc.bMaxCodecConfigDelay			= 1;
	h->desc.bmSupportedSyncFrameTypes		= 0x02;
	h->desc.bmSupportedRateControlModes		= 0x02;
	h->desc.wMaxMBperSecOneResolutionNoScalability	= 0x03FF;
	h->desc.wMaxMBperSecOneResolutionTemporalScalability	= 0x3FF;
	...
	case UVCG_FRAME: {
		...
		*size += frm->frame.b_frame_interval_type * sz;
		...
		if (frm->fmt_type != UVCG_H264) {
			...
			*size += sizeof(frm->frame_h264);
			*size += frm->frame_h264.b_num_frame_intervals * sz;
		}
#endif
	}
	break;
	...
	case UVCG_FORMAT: {
		...
		if (fmt->type == UVCG_UNCOMPRESSED) {
			...
			*dest += sizeof(m->desc);
			...
			struct uvc_format_h264 *h264 = *dest;
			struct uvcg_h264 *h =
				container_of(fmt, struct uvcg_h264, fmt);
	...
	case UVCG_FRAME: {
		...
		*dest += sz;
		...
		if (frm->fmt_type != UVCG_H264) {
			...
			sz = sizeof(frm->frame_h264);
			memcpy(*dest, &frm->frame_h264, sz);
		}
		*dest += sz;
		if (frm->fmt_type != UVCG_H264) {
			sz = frm->frame.b_frame_interval_type *
			...
			sz = frm->frame_h264.b_num_frame_intervals *
				sizeof(*frm->dw_frame_interval);
		}
		memcpy(*dest, frm->dw_frame_interval, sz);
		*dest += sz;
		...
		if (frm->fmt_type == UVCG_UNCOMPRESSED)
			...
			h->bLength = UVC15_DT_FRAME_H264_SIZE(
				frm->frame_h264.b_num_frame_intervals);
	}
	break;
	}
...
int uvcg_attach_configfs(struct f_uvc_opts *opts)
{
	...
	uvcg_init_group(&uvcg_extension_grp.group,
			uvcg_extension_default_groups,
			"extension",
