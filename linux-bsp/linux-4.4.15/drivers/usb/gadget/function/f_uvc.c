...
static bool bulk_mode= 0 ;
static bool rtos_pl = 0;
...
//	(struct usb_descriptor_header *) &uvc_bulk_streaming_intf_alt0,
		...
		switch (uvc->state) {
			case UVC_STATE_CONNECTED:
				if (!uvc->video.ep->driver_data) {
					if (uvc->video.ep) {
						ret = config_ep_by_speed(f->config->cdev->gadget,&(uvc->func), uvc->video.ep);
						if (ret)
							return ret;
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	uvc->video.bulk_mode = bulk_mode;
	uvc->video.rtos_pl = rtos_pl;
	...
	if (!bulk_mode) {
		uvc_fs_streaming_ep.wMaxPacketSize = cpu_to_le16(min(opts->streaming_maxpacket, 1023U));
		uvc_fs_streaming_ep.bInterval = opts->streaming_interval;

		uvc_hs_streaming_ep.wMaxPacketSize = cpu_to_le16(max_packet_size | ((max_packet_mult - 1) << 11));
		...
		uvc_ss_streaming_comp.wBytesPerInterval = cpu_to_le16(max_packet_size * max_packet_mult * (opts->streaming_maxburst + 1));
	} else {
		uvc_fs_bulk_streaming_ep.wMaxPacketSize = min(opts->streaming_maxpacket, 64U);
		uvc_hs_bulk_streaming_ep.wMaxPacketSize = min(opts->streaming_maxpacket, 512U);
		uvc_ss_bulk_streaming_ep.wMaxPacketSize = min(opts->streaming_maxpacket, 1024U);
		uvc_ss_bulk_streaming_comp.bMaxBurst = opts->streaming_maxburst;
		uvc_ss_bulk_streaming_comp.bmAttributes = max_packet_mult - 1;
	}
	...
	if (gadget_is_superspeed(c->cdev->gadget)) {
		INFO(cdev, "try to allocat EP superspeed\n");
		if (!bulk_mode){
		...
		INFO(cdev, "try to allocat EP highspeed\n");
		if (!bulk_mode){
		...
		INFO(cdev, "try to allocat EP fullspeed\n");
		if (!bulk_mode){
...
static struct usb_function_instance *uvc_alloc_inst(void)
{
	...
	/* A20D2423-FF3A-417D-A1F7-946A508B9267 */
	struct file * fp = NULL;
	char guid[] = {
		0x23, 0x24, 0x0D, 0xA2, 0x3A, 0xFF, 0x7D, 0x41,
		0xA1, 0xF7, 0x94, 0x6A, 0x50, 0x8B, 0x92, 0x67
	};
	printk("uvc_alloc_inst\n");

	fp = filp_open("/tmp/bulk.conf", O_RDONLY, 0);
	...
	if(IS_ERR(fp)){
        ...
        fp = filp_open("/tmp/rtospl.conf", O_RDONLY, 0);

        if(IS_ERR(fp)){
                rtos_pl = 0;
                printk("%s, not using rtos pl\n", __func__);
        }else{
                rtos_pl = 1;
                printk("%s, using rtos pl\n", __func__);
        }
	...
	ed->bUnitID			= 3;
	memcpy(ed->guidExtensionCode, guid, sizeof(guid));
	ed->bNumControls		= 1;
	ed->bNrInPins			= 1;
	ed->baSourceID[0]		= 2;
	ed->bControlSize		= 2;
