...
static int
uvc_set_user_param(struct uvc_device *uvc, struct uvc_user_param *data)
{
	struct uvc_video *video = &uvc->video;

	memcpy(&video->user_param, data, sizeof(video->user_param) );
	video->max_payload_size = data->max_payload_size;

	printk( "DEBUG_GNSO: 0x%x, 0x%x, 0x%x\n", data->with_header, data->header_size, data->max_payload_size );
	printk( "DEBUG_GNSO: %d\n", sizeof(video->user_param));
	return 0;
}
		       ...
		       unsigned int cmd, void *arg)
{
	...
	case UVCIOC_SET_USER_PARAM:
		return uvc_set_user_param(uvc, arg);
	default:
		return -ENOIOCTLCMD;
	}
}
