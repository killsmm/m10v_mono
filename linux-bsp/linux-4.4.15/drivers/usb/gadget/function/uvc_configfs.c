...
/* streaming/uncompressed_nv12 */
static struct uvcg_uncompressed_nv12_grp {
	struct config_group	group;
} uvcg_uncompressed_nv12_grp;
...
static struct config_item *fmt_parent[] = {
	...
	&uvcg_uncompressed_nv12_grp.group.cg_item,
};
...
static struct config_group *uvcg_uncompressed_nv12_make(struct config_group *group,
						   const char *name)
{
	static char guid[] = {
...
static struct configfs_group_operations uvcg_uncompressed_nv12_grp_ops = {
	.make_group		= uvcg_uncompressed_nv12_make,
	.drop_item		= uvcg_uncompressed_drop,
};

static struct config_item_type uvcg_uncompressed_nv12_grp_type = {
	.ct_group_ops	= &uvcg_uncompressed_nv12_grp_ops,
	.ct_owner	= THIS_MODULE,
};
...
UVCG_H264_ATTR(b_bits_per_pixel, bBitsPerPixel, identity_conv);
UVCG_H264_ATTR(b_default_frame_index, bDefaultFrameIndex,
		       identity_conv);
UVCG_H264_ATTR_RO(b_aspect_ratio_x, bAspectRatioX, identity_conv);
UVCG_H264_ATTR_RO(b_aspect_ratio_y, bAspectRatioY, identity_conv);
			    ...
			    enum uvcg_strm_type type)
{
	...
	**array = *dest;
	...
	case UVCG_FRAME: {
		...
		/* H.264/FRAME_BASED packet is slightly different */
		if (frm->fmt_type == UVCG_H264) {
			struct h264_frame_fields hf;
			size_t l;
			hf.b_frame_interval_type = frm->frame.b_frame_interval_type;
			hf.dw_default_frame_interval = frm->frame.dw_default_frame_interval;
			hf.dw_bytes_per_line = 0;
			l = (size_t)&frm->frame.dw_max_video_frame_buffer_size;
			l = l - (size_t)&frm->frame;
			memcpy(*dest + l, &hf, sizeof(hf));
		}
		...
		*dest += sz;
		if (frm->fmt_type == UVCG_UNCOMPRESSED)
			...
			h->bLength = UVC_DT_FRAME_H264_SIZE(
				frm->frame.b_frame_interval_type);
	}
	break;
	}
...
static struct config_group *uvcg_streaming_default_groups[] = {
	...
	&uvcg_uncompressed_nv12_grp.group,
	NULL,
};
...
int uvcg_attach_configfs(struct f_uvc_opts *opts)
{
	...
	config_group_init_type_name(&uvcg_uncompressed_nv12_grp.group,
				    "uncompressed_nv12",
				    &uvcg_uncompressed_nv12_grp_type);
