...
static long uac_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case UACIOC_STREAMON:
		u_audio_start_playback(audio);
		usb_composite_setup_continue(audio->func.config->cdev);
		audio->state = UAC_STATE_STREAMING;
		break;

	case UACIOC_STREAMOFF:
		u_audio_stop_playback(audio);
		break;
...
static struct uac1_ac_header_descriptor_1 ac_header_desc3 = {
	.bLength =		UAC_DT_AC_HEADER_LENGTH,
	.bDescriptorType =	USB_DT_CS_INTERFACE,
	.bDescriptorSubtype =	UAC_HEADER,
	.bcdADC =		cpu_to_le16(0x0100),
...
static struct usb_descriptor_header *f_audio_desc3[] = {
	(struct usb_descriptor_header *)&iad_desc,

	(struct usb_descriptor_header *)&ac_interface_desc,
	(struct usb_descriptor_header *)&ac_header_desc3,
...
static struct usb_descriptor_header *ss_f_audio_desc3[] = {
	(struct usb_descriptor_header *)&iad_desc,
...
static void uac_cs_attr_sample_rate(struct usb_ep *ep, struct usb_request *req)
{
	...
	printk("%s, %d, ctl_id=%x, val:%x\n", __func__, __LINE__, uac1->ctl_id, val);
	if(uav_mode==1){
		if (uac1->ctl_id == (USB_DIR_IN | 3)) {
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	printk("%s, bRequest 0x%x, w_value 0x%04x, len %d, endpoint %x\n",
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	printk("cas bRequest 0x%x, w_value 0x%04x, len %d, endpoint %d\n",
...
f_audio_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	printk("%s, %d, w_index:%x, w_value:%x, w_length:%x, ctrl->bRequestType:%x\n", __func__, __LINE__, w_index, w_value, w_length, ctrl->bRequestType);
...
static int f_audio_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	printk("%s, %d, alt:%d, intf:%d , uac1->ac_intf:%d, uac1->as_in_intf:%d\n", __func__, __LINE__, alt, intf, uac1->ac_intf, uac1->as_in_intf);
...
static int f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (!ep)
		goto fail;
#if 0 //test
	printk("%s, %d , assign ep addr:%x\n", __func__, __LINE__, ep->address);
	as_in_ep_desc.bEndpointAddress = ep->address;
#endif
	audio->in_ep = ep;
	...
	if(uav_mode){
		printk("%s, %d , uav mode\n", __func__, __LINE__);
		iad_desc.bFirstInterface= 2;
		/* copy descriptors, and track endpoint copies */
		status = usb_assign_descriptors(f, f_audio_desc3, f_audio_desc3, ss_f_audio_desc3);
		if (status)
			goto fail;
	}else{
		printk("%s, %d , uac normal\n", __func__, __LINE__);
		iad_desc.bFirstInterface= 0;
		/* copy descriptors, and track endpoint copies */
...
#if 1 // test
{
	struct file *fp = filp_open("/tmp/1011.conf", O_RDONLY, 0);

	if(IS_ERR(fp)){
		uav_mode = 0;
		printk("%s, uac normal mode\n", __func__);
	}else{
		uav_mode = 1;
		printk("%s, uav mode\n", __func__);
	}
