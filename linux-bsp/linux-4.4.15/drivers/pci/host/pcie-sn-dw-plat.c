...
#else
#define     BASE_CLASS_CODE_VALUE       0x05
#endif
...
#else
#define     SUBCLASS_CODE_VALUE         0x80
#endif
...
#define   AXI_CLK_222	      (BIT(11) | BIT(10) | BIT(9) | BIT(8) | BIT(7) | BIT(6) | BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0))
...
#ifdef CONFIG_M10V_AS_EP
#define BUSCKSTP        0x60
#define  PCIE0_CKSTP    BIT(1)
#define RAMPD 0x40
#define  RAMPD_PCIE0    BIT(8)

#define INT_CONTROL_2       0x28
#define   SEND_COR_ERR_EN       BIT(0)
#define   SEND_COR_ERR_CEN      BIT(1)
#define   SEND_COR_ERR_M        BIT(3)
#define   SEND_NF_ERR_EN        BIT(4)
#define   SEND_NF_ERR_CEN       BIT(5)
#define   SEND_NF_ERR_M         BIT(7)
#define   SEND_F_ERR_EN         BIT(8)
#define   SEND_F_ERR_CEN        BIT(9)
#define   SEND_F_ERR_M          BIT(11)
#define   RDLH_LINK_UP_EN       BIT(16)
#define   RDLH_LINK_UP_CEN      BIT(17)
#define   RDLH_LINK_UP_M        BIT(19)
#endif
...
struct sn_dw_plat_pcie *g_sn;

#ifdef CONFIG_M10V_AS_EP
static int msi_trigger(struct sn_dw_plat_pcie *sn, unsigned int reg)
{
        u8 bme, msie, mme;/*bit2, BIT16, BIT20-22*/

        pr_info("milb: msi_trigger(), reg = %02x\n", reg);

        bme = readb(sn->ctrl_base + 0x4);
        if((bme & BIT(2)) == 0)
                writeb(bme|BIT(2), sn->ctrl_base + 0x4);

        msie = readb(sn->ctrl_base + 0x52);
        if( (msie & BIT(0)) == 0)
                writeb( msie|BIT(0), sn->ctrl_base + 0x52);

        mme = readb(sn->ctrl_base + 0x52);
        writeb(mme|BIT(4)|BIT(5)|BIT(6), sn->ctrl_base + 0x52);

        writel((unsigned int)1<<reg, sn->exs_base + 0x01c);

        writeb(bme, sn->ctrl_base + 0x4);
        writeb(msie, sn->ctrl_base + 0x52);

	return 0;
}
static ssize_t msitra_start(struct device *d, struct device_attribute *attr, const char *buf, size_t count)
{
        unsigned int reg;
	u8 bme, msie, mme;/*bit2, BIT16, BIT20-22*/
	struct sn_dw_plat_pcie *sn = dev_get_drvdata(d);

	sscanf(buf, "%x", &reg);
	pr_info("milb: msitra(), reg = %02x\n", reg);
...
static DEVICE_ATTR(msitra, S_IWUSR | S_IRUGO, NULL, msitra_start);

static struct attribute* milb_attributes[] = {
        &dev_attr_msitra.attr,
        NULL
};
...
static void sn_dbi_ro_wr_en(void __iomem *ctrl_base, void __iomem *exs_base)
{
...
//	writel(0xff, ctrl_base+0x9c8);
//	writel(0xff, ctrl_base+0xa1c);
...
static int sn_pcie_init(void __iomem *ctrl_base, void __iomem *exs_base)
{
	...
	printk("milb: %s\n", __func__);
#ifdef CONFIG_M10V_AS_EP
	sn_pcie_set_data(exs_base, RAMPD, RAMPD_PCIE0, 0);
#endif
...
#ifndef CONFIG_M10V_AS_EP
	sn_pcie_set_data(exs_base, RESET_CONTROL_1, PERST_N_O_REG , 0);/* marked, only use in rc*/
#endif
	...
	sn_pcie_set_data(exs_base, CORE_CONTROL, DEVICE_TYPE, 0);//TO: SET TO 0
#endif

	/* 4: Set Bifurcation  1=disable  2=enable */
	sn_pcie_set_data(exs_base, PHY_CONFIG_COM_6, PIPE_PORT_SEL, 2);

	/* 4.5 set lane 0x1: (x1); 0x11(x2) */
	//sn_pcie_set_data(ctrl_base, 0x710, BIT(16) | BIT(17), 0x1);//boot failed
	/* 00001(x1); 00010(x2) */
	//sn_pcie_set_data(ctrl_base, 0x80c, BIT(8) | BIT(9), 0x1);

	//sn_pcie_set_data(ctrl_base, 0x8c0, BIT(0), 0x1);
...
#ifndef CONFIG_M10V_AS_EP
	sn_pcie_set_data(exs_base, RESET_CONTROL_1, PERST_N_O_REG, 1);/* RC only*/
#endif
...
#else
	sn_pcie_set_data(exs_base, AXI_CLK_STOP, AXI_CLK_222, 222);
#endif
...
#ifdef CONFIG_M10V_AS_EP
	sn_pcie_set_data(exs_base, RESET_CONTROL_1, BIT(0) | BIT(1), 3);
	sn_pcie_set_data(exs_base, RESET_CONTROL_1, BIT(4) | BIT(5), 3);
#endif
	...
	sn_dbi_ro_wr_en(ctrl_base, exs_base);
...
//	writeb(0, ctrl_base + 0x903);//set as ob, but default value is 0, should be removed.
...
//#endif
#ifdef CONFIG_M10V_AS_EP
static irqreturn_t dma_isr(int irq, void *arg){
        u32 wintr, rintr;

        wintr = readl(g_sn->ctrl_base + DMA_WRITE_INT_STATUS_OFF);
        rintr = readl(g_sn->ctrl_base + DMA_READ_INT_STATUS_OFF);

	pr_info("milb:%s() wintr:%08x, rintr:%08x \n", __func__, wintr, rintr);

        if(wintr & BIT(0)){
                msi_trigger(g_sn, 1);
                pr_info("milb: wintr \n");
		writel(0xFFFFFFFF, g_sn->ctrl_base + DMA_WRITE_INT_MASK_OFF);
                //writel(0xFFFFFFFF, g_sn->ctrl_base + 0x9c8);
        }

        if(rintr & BIT(0)){
                msi_trigger(g_sn, 1);
                pr_info("milb: rintr");
		writel(0xFFFFFFFF, g_sn->ctrl_base + DMA_READ_INT_MASK_OFF);
                //writel(0xFFFFFFFF, g_sn->ctrl_base + 0xa1c);
        }
...
static void msi_poll(struct work_struct *work)
{
        struct sn_dw_plat_pcie *sn =
                        container_of(work, struct sn_dw_plat_pcie,
                                     work.work);
	u32 wintr, rintr;

	wintr = readl(sn->ctrl_base + DMA_WRITE_INT_STATUS_OFF);
	rintr = readl(sn->ctrl_base + DMA_READ_INT_STATUS_OFF);
	...
	if(wintr & BIT(0)){
		msi_trigger(sn, 1);
		pr_info("milb: wintr \n");
		//writel(0xFFFFFFFF, sn->ctrl_base + 0x9c8);
	}

	if(rintr & BIT(0)){
		msi_trigger(sn, 1);
		pr_info("milb: rintr");
		//writel(0xFFFFFFFF, sn->ctrl_base + 0xa1c);
	}

        schedule_delayed_work(&sn->work, HZ / 100);
...
#ifdef CONFIG_M10V_AS_EP
        ret = sysfs_create_group(&pdev->dev.kobj, &milb_group);
        if(ret)
                pr_info("milb: can't create sysfs \n");

	INIT_DELAYED_WORK(&sn_dw_plat_pcie->work, msi_poll);
	//schedule_delayed_work(&sn_dw_plat_pcie->work, HZ / 2);//msi polling only.
#endif
...
//#endif
        ...
        pr_info("milb: dma irq:%d\n", irq);
        if (irq < 0)
                pr_info("milb: dma irq request failed \n");

        ret = request_irq(irq, dma_isr, IRQF_SHARED, "milb_dma_irq", pp);
#endif
