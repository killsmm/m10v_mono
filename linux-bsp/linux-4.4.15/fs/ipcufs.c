...
static void ipcufs_alloc_drive(u32 s)
{
	...
	while(!fake_file[s])
	{
		fake_file[s] = kzalloc(sizeof(struct fake_inode) * inode_num, GFP_NOWAIT);
		printk("%s, Can not allocate fake_file Buffer, retry again\n", __func__);

		if(retry++ > 5)
			printk("%s, Can not allocate fake_file Buffer more than 5 times\n", __func__);
	}
		       ...
		       int rw)
{
	...
	while(!it)
	{
		...
		printk("%s, Can not allocate it Buffer, retry again\n", __func__);
...
static int ipcufs_file_overwrite(struct file *file)
{
	...
	while(!buf)
	{
		...
		printk("%s, Can not allocate buf Buffer, retry again\n", __func__);
				    ...
				    unsigned int flags)
{
	...
	while(!tmp_inode)
	{
		...
		printk("%s, tmp_inode - Can not allocate buf Buffer, retry again\n", __func__);

		if(retry++ > 5)
			printk("%s - Can not allocate tmp_inode Buffer more than 5 times\n", __func__);
	}
	retry = 0;
	while(!buf)
	{
		buf = kzalloc(IPCUFS_MAX_PATH_LEN, GFP_NOWAIT);
		printk("%s, buf - Can not allocate buf Buffer, retry again\n", __func__);

		if(retry++ > 5)
			printk("%s - Can not allocate buf Buffer more than 5 times\n", __func__);
	}
	path_in = dentry_path_raw(dentry, buf, IPCUFS_MAX_PATH_LEN);
...
static int ipcufs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	path_in = dentry_path_raw(file->f_path.dentry, buf, IPCUFS_MAX_PATH_LEN);
			    ...
			    struct list_head *pages, unsigned nr_pages)
{
	...
	{
		it = kzalloc(sizeof(struct ipcufs_info_table), GFP_NOWAIT);
		printk("%s,%d - Can not allocate buf Buffer, retry again\n", __func__, __LINE__);
			 ...
			 struct inode *inode_new, struct dentry *dentry_new)
{
	...
	path_in_new = dentry_path_raw(dentry_new, path_buf_new, IPCUFS_MAX_PATH_LEN);
...
static void ipcufs_umount_begin(struct super_block *sb)
{
	...
	while(!it)
	{
		...
		if(retry++ > 5)
			printk("%s,%d - Can not allocate it Buffer more than 5 times\n", __func__, __LINE__);
...
static int sn_ipcufs_probe(struct platform_device *pdev)
{
	...
	while(!fake_file)
	{
		fake_file =  kzalloc(sizeof(struct fake_inode *) * IPCUFS_MAX_DRV_NUM, GFP_NOWAIT);
		printk("%s,%d - Can not allocate buf Buffer, retry again\n", __func__, __LINE__);
